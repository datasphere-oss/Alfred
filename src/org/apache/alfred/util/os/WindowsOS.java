...
import org.apache.alfred.util.io.ProcessHelper;
...
public final class WindowsOS {
    ...
    public static void createShortcut(File target, File location, File icon) throws IOException {

        createShortcut(target, location.getAbsolutePath(), icon.getAbsolutePath());
    }

    @SuppressFBWarnings("COMMAND_INJECTION")
    public static void createShortcut(File target, String location, String icon) throws IOException {
        ...
        try {
            ...
            writer.write(String.format(mkshortcut, location, StringUtils.quote(target.getAbsolutePath()),
                    StringUtils.quote(target.getParentFile().getAbsolutePath()), StringUtils.quote(icon)));
            writer.close();
            writer = null;
        } finally {
            Util.close(writer);
    ...
    public static String getHardDiskSerial() {
        ...
        try {
            File file = File.createTempFile("getHardDiskSerial", ".vbs");
            file.deleteOnExit();
    ...
    public static int start(String cmd) throws IOException, InterruptedException {
        return (ProcessHelper.execAndWait("cmd", "/c", "start", cmd));
    }

    public static String cppRuntimeVersion(int runtimeNumber, boolean isx64) throws IllegalArgumentException {
        String value = WindowsUtils.regQuery(HKey.HKLM, "SOFTWARE\\Wow6432Node\\Microsoft\\VisualStudio\\"
                + runtimeNumber + ".0\\VC\\Runtimes\\" + (isx64 ? "x64" : "x86"), "Version");
        return value;
    }
