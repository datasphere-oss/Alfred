...
public class WatchdogService {
    ...
    public final static File SERVICE_CONFIG_BIN_INST = Home
            .getFile("../Utilities/DXWatchdog/" + SERVICE_NAME + ".exe.config");
    ...
    public boolean register(String service, int restartDelay, int restartAttempts) {
        ...
        return sendCommand(
                "REG " + service + "," + String.valueOf(restartDelay) + "," + String.valueOf(restartAttempts));
    }
    ...
    private boolean installFiles() {
        ...
        try {
            ...
            if (Util.IS32BIT) {
                ...
                Files.copy(SERVICE_CONFIG_BIN32.toPath(), SERVICE_CONFIG_BIN_INST.toPath(),
                        StandardCopyOption.REPLACE_EXISTING);
            } else {
                Files.copy(SERVICE_BIN.toPath(), SERVICE_BIN_INST.toPath(), StandardCopyOption.REPLACE_EXISTING);
                Files.copy(SERVICE_CONFIG_BIN.toPath(), SERVICE_CONFIG_BIN_INST.toPath(),
                        StandardCopyOption.REPLACE_EXISTING);
            }
        } catch (IOException e) {
    ...
    public boolean install() {
        ...
        try {
            ServiceControlFactory.getInstance().create(SERVICE_NAME, SERVICE_DESC, SERVICE_BIN_INST.getCanonicalPath(),
                    params);
        } catch (Exception e) {
    ...
    public boolean running() {
        ...
        try {
            running = ServiceControlFactory.getInstance().queryStatusName(SERVICE_NAME)
    ...
    static public void main(String[] args) {
        ...
        System.out.format("Deltix Service Watchdog Console v. %.2f (try 'help' for information)",
                WatchdogService.INSTANCE.VERSION);
        String request = "";
        do {
            try {
                System.out.print("\n >");
