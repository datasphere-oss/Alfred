...
package org.apache.alfred.util.service;

import org.apache.alfred.util.jni.WindowsService;
...
public abstract class BaseService extends WindowsService {
    ...
    private void stop(int errorCode) {
        try {
            ...
            // if ((errorCode == 0 || errorCode == ERROR_CODE) &&
            // WatchdogService.INSTANCE.running())
    ...
    /**
     * When implemented in a derived class, OnCustomCommand(int) executes when the
     * Service Control Manager (SCM) passes a custom command to the service.
     * Specifies actions to take when a command with the specified parameter value
     * occurs.
     * 
     * @param command The command message sent to the service (see
     *                WindowsService.SERVICE_CONTROL_*)
     * @throws Exception
     */
