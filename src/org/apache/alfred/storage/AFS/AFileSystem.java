...
import org.apache.alfred.storage.afs.exceptions.AfsExceptionUnresolvableRoot;
...
public interface AFileSystem extends AResolving, WriteController {
	...
	/**
	 * Ensures the default root directory. May not be supported by different file
	 * system implementations.
	 * 
	 * @return the root directory
	 * @throws UnsupportedOperationException if the file system doesn't have a
	 *                                       default root
	 */
	...
	/*
	 * Default implementation assumes items can be handled in a unified way. If not,
	 * the interface allows for switching it around.
	 */
	...
	public abstract class Abstract<H extends AIoHandler, D, F> implements AFileSystem, AResolver<D, F>, ACreator {
		...
		protected Abstract(final String defaultProtocol, final H ioHandler) {
			this(defaultProtocol, null, ioHandler);
		}

		protected Abstract(final String defaultProtocol, final ACreator.Creator creatorCreator, final H ioHandler) {
			this(defaultProtocol, creatorCreator, AccessManager::New, ioHandler);
		}

		protected Abstract(final String defaultProtocol, final ACreator.Creator creatorCreator,
				final AccessManager.Creator accessManagerCreator, final H ioHandler) {
			super();
			...
			// called at the very last just in case the creator needs some of the other
			// state
		...
		public final synchronized <I extends AItem> I validateMember(final I item) {
			...
			throw new AfsExceptionConsistency("Incompatible parent FileSystem of " + XChars.systemString(item) + ":"
		...
		private boolean validateRegisteredRootDirectory(final ADirectory rootDirectory) {
			...
			throw new AfsExceptionConsistency("Inconsistent root directories for identifier \"" + rootIdentifier
		...
		@Override
		public ADirectory ensureDefaultRoot() {
			throw new UnsupportedOperationException("This file system implementation (" + this.getClass().getName()
		...
		@Override
		public final synchronized ADirectory resolveDirectoryPath(final String[] pathElements, final int offset,
				final int length) {
		...
		@Override
		public final synchronized ADirectory ensureDirectoryPath(final String[] pathElements, final int offset,
				final int length) {
			if (length <= 0) {
		...
		@Override
		public final synchronized AFile ensureFilePath(final String[] directoryPathElements, final int offset,
				final int length, final String fileIdentifier) {
		...
		@Override
		public ARoot createRootDirectory(final AFileSystem fileSystem, final String protocol, final String identifier) {
