...
import org.apache.alfred.core.X;
...
import org.apache.alfred.core.util.UtilStackTrace;

public interface AIoHandler extends WriteController {
	/*
	 * (08.06.2020 TM)TODO: priv#49: JavaDoc: guaranteed completeness guarantees
	 * that all specified bytes (where applicable) are read/written. Any iterative,
	 * reattempting, waiting, timeouting etc. logic that is required for this must
	 * be covered by the IoHandler implementation.
	 */

	/*
	 * (16.07.2020 TM)TODO: priv#49: composite IoHandler The default implementation
	 * is rather strict on its handleable file and directory implementations. This
	 * is intentional. If a more flexible or dynamic/heuristic approach is desired,
	 * it has to be implemented as another IoHandler. For example a "composite"
	 * IoHandler that looks up the specific IoHandler depending on the
	 * implementation of a passed file or directory.
	 */

	/*
	 * (16.07.2020 TM)TODO: priv#49: target positional parameters So far, variants
	 * for target positional writing (including copying) is missing completely.
	 * There is a way to provide a source position, but the target can only be
	 * written to in the way the implementation has set the default behavior (always
	 * append or always start at 0). This matches the microstream strategy, but in
	 * general, it must be possible to write to a specified position in the target
	 * file. Simple example: a utility that replaces found byte ranges with a new
	 * value (search & replace). Currently, such a tricial functionality is not
	 * possible with AFS.
	 */
	...
	// public long copyTo(AReadableFile sourceSubject, AWritableFile target, long
	// targetPosition);
	//
	// public long copyTo(AReadableFile sourceSubject, AWritableFile target, long
	// targetPosition, long length);
	//
	// public long copyTo(AReadableFile sourceSubject, long srcPos, AWritableFile
	// target, long trgPos, long length);
	...
	// public long copyFrom(AReadableFile source, AWritableFile targetSubject, long
	// targetPosition);
	//
	// public long copyFrom(AReadableFile source, AWritableFile targetSubject, long
	// targetPosition, long length);
	//
	// public long copyFrom(AReadableFile source, long srcPos, AWritableFile
	// targetSubject, long trgPos, long length);
	...
	public abstract class Abstract<FS, DS, I extends AItem, F extends AFile, D extends ADirectory, R extends AReadableFile, W extends AWritableFile>
		...
		protected Abstract(final WriteController writeController, final Class<I> typeItem, final Class<F> typeFile,
				final Class<D> typeDirectory, final Class<R> typeReadableFile, final Class<W> typeWritableFile) {
			super();
		...
		//
		// protected abstract long specificCopyTo(R sourceSubject, AWritableFile target,
		// long targetPosition, long length);
		//
		// protected abstract long specificCopyTo(R sourceSubject, long srcPos,
		// AWritableFile target, long trgPos, long length);
		...
		protected abstract long specificCopyFrom(AReadableFile source, long sourcePosition, long length,
				W targetSubject);

		// protected abstract long specificCopyFrom(AReadableFile source, W
		// targetSubject, long targetPosition);
		//
		// protected abstract long specificCopyFrom(AReadableFile source, W
		// targetSubject, long targetPosition, long length);
		//
		// protected abstract long specificCopyFrom(AReadableFile source, long srcPos, W
		// targetSubject, long trgPos, long length);
		...
		protected long copyGeneric(final AReadableFile source, final AWritableFile target) {
		...
		protected long copyGeneric(final AReadableFile source, final long sourcePosition, final AWritableFile target) {
			final long sourceSize = source.size(); // explicit variable for debugging purposes
		...
		protected RuntimeException createUnhandledTypeException(final Object subject, final Class<?> checkedType,
			...
			return UtilStackTrace
					.cutStacktraceByN(
							new AfsExceptionConsistency("Instance is not of type \"" + checkedType
		...
		public boolean exists(final ADirectory directory) {
			if (directory.parent() == null && directory.identifier().isEmpty()) {
				// an empty-named root directory (= leading separator) is assumed implicitely
		...
		public void create(final ADirectory directory) {
			...
			synchronized (ADirectory.actual(directory)) {
				...
				// it is up to the specific implementation to decide how to handle root
				// directories
		...
		@Override
		public long copyTo(final AReadableFile sourceSubject, final AWritableFile target) {
			if (sourceSubject.fileSystem() != target.fileSystem() || !this.isHandledReadableFile(sourceSubject)
					|| !this.isHandledWritableFile(target)) {
				// it is by far the most common and intuitive case for copy to ensure existence
				// implicitely
				target.ensureExists();
		...
		@Override
		public long copyTo(final AReadableFile sourceSubject, final long sourcePosition, final AWritableFile target) {
			if (sourceSubject.fileSystem() != target.fileSystem() || !this.isHandledReadableFile(sourceSubject)
					|| !this.isHandledWritableFile(target)) {
				// it is by far the most common and intuitive case for copy to ensure existence
				// implicitely
		...
		@Override
		public long copyTo(final AReadableFile sourceSubject, final long sourcePosition, final long length,
		...
		// this.validateHandledReadableFile(sourceSubject);
		//
		// // it is by far the most common and intuitive case for copy to ensure
		// existence implicitely
		// target.ensureExists();
		//
		// return this.specificCopyTo(this.typeReadableFile.cast(sourceSubject), target,
		// targetPosition);
		...
		// this.validateIsWritable();
		//
		// return this.specificCopyTo(this.typeReadableFile.cast(sourceSubject),
		// sourcePosition, target, targetPosition, length);
		// }

		@Override
		public long copyFrom(final AReadableFile source, final AWritableFile targetSubject) {
			if (source.fileSystem() != targetSubject.fileSystem() || !this.isHandledReadableFile(source)
		...
		@Override
		public long copyFrom(final AReadableFile source, final long sourcePosition, final AWritableFile targetSubject) {
			if (source.fileSystem() != targetSubject.fileSystem() || !this.isHandledReadableFile(source)
					|| !this.isHandledWritableFile(targetSubject)) {
				// it is by far the most common and intuitive case for copy to ensure existence
				// implicitely
		...
		@Override
		public long copyFrom(final AReadableFile source, final long sourcePosition, final long length,
		...
		// this.validateIsWritable();
		//
		// return this.specificCopyFrom(source,
		// this.typeWritableFile.cast(targetSubject), targetPosition);
		// }
		//
		// @Override
		// public long copyFrom(
		...
		//
		// return this.specificCopyFrom(source,
		// this.typeWritableFile.cast(targetSubject), targetPosition, length);
		// }
		//
		// @Override
		// public long copyFrom(
		...
		// this.validateIsWritable();
		//
		// return this.specificCopyFrom(source, sourcePosition,
		// this.typeWritableFile.cast(targetSubject), targetPosition, length);
		// }
		...
		public long writeBytes(final AWritableFile targetFile, final Iterable<? extends ByteBuffer> sourceBuffers) {
			...
			synchronized (targetFile.actual()) {
				...
				final long writeCount = this.specificWriteBytes(this.typeWritableFile.cast(targetFile), sourceBuffers);
		...
		public void moveFile(final AWritableFile sourceFile, final AWritableFile targetFile) {
			...
			/*
			 * intentionally no locking here, since there are TWO sets of parent&file
			 * involved, which could cause deadlocks.
			 */
