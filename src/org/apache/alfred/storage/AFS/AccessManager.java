...
import static org.apache.alfred.X.notNull;

import java.util.function.Function;

import org.apache.alfred.X;
import org.apache.alfred.storage.afs.exceptions.AfsExceptionConsistency;
import org.apache.alfred.storage.afs.exceptions.AfsExceptionExclusiveAttemptConflict;
import org.apache.alfred.storage.afs.exceptions.AfsExceptionExclusiveAttemptSharedUserConflict;
import org.apache.alfred.storage.afs.exceptions.AfsExceptionMutation;
import org.apache.alfred.storage.afs.exceptions.AfsExceptionMutationInUse;
import org.apache.alfred.storage.afs.exceptions.AfsExceptionSharedAttemptExclusiveUserConflict;
import org.apache.alfred.core.chars.XChars;
import org.apache.alfred.core.collections.HashTable;
import org.apache.alfred.core.collections.XArrays;
import org.apache.alfred.core.collections.interfaces.OptimizableCollection;
...
public interface AccessManager {
	...
	public <R> R executeMutating(ADirectory directory, Function<? super ADirectory, R> logic);
	...
	public static AccessManager New(final AFileSystem fileSystem) {
		return new AccessManager.Default<>(notNull(fileSystem));
	}
	...
	public class Default<S extends AFileSystem> implements AccessManager {
		...
		static final class FileEntry {
			...
			final boolean removeShared(final AReadableFile file) {
				...
				if (this.sharedUsers[index] != file) {
					throw new AfsExceptionConsistency("Inconsistency detected: to be removed file \"" + file
							+ "\" is not the same as the one contained for the same user: \"" + this.sharedUsers[index]
							+ "\".");
				}
		...
		@Override
		public boolean isUsedReading(final AFile file, final Object user) {
			synchronized (this.mutex()) {
				final FileEntry e = this.fileUsers.get(AFile.actual(file));
				if (e == null) {
					return false;
		...
		@Override
		public boolean isUsedWriting(final AFile file, final Object user) {
			synchronized (this.mutex()) {
				final FileEntry e = this.fileUsers.get(AFile.actual(file));
				if (e == null) {
					return false;
		...
		@Override
		public final <R> R executeMutating(final ADirectory directory, final Function<? super ADirectory, R> logic) {
			synchronized (this.mutex()) {
				...
				if (mutatingThread != null) {
					throw new AfsExceptionMutationInUse("Directory \"" + directory.toPathString()
							+ "\" already used for mutation by \"" + mutatingThread + "\".");
				}
		...
		private AReadableFile registerReading(final FileEntry entry, final AFile actual, final Object user) {
		...
		private AReadableFile registerReading(final AFile actual, final Object user) {
		...
		private AReadableFile convertToReading(final AWritableFile file) {
		...
		protected final void incrementDirectoryUsageCount(final ADirectory directory) {
			...
			// note: child count incrementation on one level does not concern the parent
			// directory count.
		}
		...
		public AReadableFile downgrade(final AWritableFile file) {
			synchronized (this.mutex()) {
				...
				// may not retire file before conversion since that might need some of file's
				// state.
				e.exclusive = null;
		...
		protected final AReadableFile internalUseReading(final AFile file, final Object user,
		...
		protected final AWritableFile internalUseWriting(final AFile file, final Object user,
				final ConflictHandler conflictHandler) {
			final AFile actual = AFile.actual(file);
			final FileEntry e = this.fileUsers.get(actual);
			if (e == null) {
		...
		private static final ConflictHandler CONFLICT_HANDLER_NO_OP = new ConflictHandler() {
			@Override
			public void handleSharedAttemptExclusiveUserConflict(final AFile actual, final Object user,
					final FileEntry entry) {
				// no-op
			}

			@Override
			public void handleExclusiveAttemptConflict(final AFile actual, final Object user, final FileEntry entry) {
				// no-op
			}

			@Override
			public void handleExclusiveAttemptSharedUsersConflict(final AFile actual, final Object user,
					final FileEntry entry) {
				// no-op
			}
		};
		...
		static String toStringWithIdentity(final Object o) {
			return o == null ? null : "(" + XChars.systemString(o) + ") " + o.toString();
		}
		...
		private static final ConflictHandler CONFLICT_HANDLER_EXCEPTION = new ConflictHandler() {
			...
			public void handleSharedAttemptExclusiveUserConflict(final AFile actual, final Object user,
				...
				throw new AfsExceptionSharedAttemptExclusiveUserConflict(
						"File is already exclusively used by a different user: " + actual + ". Exclusive user: "
								+ toStringWithIdentity(entry.exclusive.user()) + ". Attempting user: "
								+ toStringWithIdentity(user) + ".");
			}

			@Override
			public void handleExclusiveAttemptConflict(final AFile actual, final Object user, final FileEntry entry) {
				throw new AfsExceptionExclusiveAttemptConflict("File is already used by a different exclusive user: "
						+ actual + ". Exclusive user: " + toStringWithIdentity(entry.exclusive.user())
			...
			public void handleExclusiveAttemptSharedUsersConflict(final AFile actual, final Object user,
					final FileEntry entry) {
				throw new AfsExceptionExclusiveAttemptSharedUserConflict("File \"" + actual.toPathString()
		...
		private AWritableFile registerWriting(final AFile actual, final Object user) {
			this.checkForMutatingParents(actual, user);
			this.incrementDirectoryUsageCount(actual.parent());
		...
		private AWritableFile convertToWriting(final AReadableFile file) {
			this.checkForMutatingParents(file.actual(), file.user());
			this.incrementDirectoryUsageCount(file.actual().parent());
		...
		private void checkForMutatingParents(final AFile actual, final Object user) {
			for (ADirectory p = actual.parent(); p != null; p = p.parent()) {
				...
				if (mutatingThread != null && mutatingThread != user) {
					throw new AfsExceptionMutation("File \"" + actual.toPathString()
							+ "\" cannot be accessed by user \"" + user + "\" since directory \"" + p.toPathString()
							+ "\" is in the process of being changed by user thread \"" + mutatingThread + "\".");
				}
			}
		}
		...
		protected boolean internalUnregister(final AReadableFile file, final FileEntry entry) {
			...
			// AWritableFile "is a" AReadableFile, so it could be passed here and must be
			// covered in any case.
		...
		protected static void throwUnregisteredException(final AReadableFile file, final FileEntry entry) {
			throw new AfsExceptionConsistency(
					"Inconsistency detected: attempting to unregister non-retired but not registered file \"" + file
							+ "\".");
		}
