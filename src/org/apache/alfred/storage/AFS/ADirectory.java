...
import static org.apache.alfred.core.X.mayNull;
...
import org.apache.alfred.storage.afs.exceptions.AfsExceptionUnresolvablePathElement;
import org.apache.alfred.core.chars.VarString;
import org.apache.alfred.core.collections.EqHashTable;
import org.apache.alfred.core.collections.XArrays;
import org.apache.alfred.core.collections.types.XGettingEnum;
import org.apache.alfred.core.collections.types.XGettingTable;
import org.apache.alfred.core.functional.XFunc;
import org.apache.alfred.core.typing.XTypes;
...
public interface ADirectory extends AItem, AResolving {
	...
	public <S, R> R accessDirectories(S subject,
			BiFunction<? super XGettingTable<String, ? extends ADirectory>, S, R> logic);
	...
	/**
	 * Removes all child items ({@link ADirectory} or {@link AFile}) that have no
	 * physical equivalent.
	 * 
	...
	/*
	 * (03.06.2020 TM)TODO: priv#49: directory mutation: - move directory to target
	 * directory - delete directory - rename directory
	 * 
	 * Each is only allowed if there are no uses for that directory
	 * 
	 * (19.07.2020 TM): Downgraded to T0D0 since MicroStream does not require
	 * directory mutations (for now...).
	 */
	...
	public abstract class Abstract extends AItem.Abstract implements ADirectory {
		///////////////////////////////////////////////////////////////////////////
		// static fields //
		//////////////////
		...
		// memory-optimized array because there should usually be no or very few
		// observers (<= 10).
		private ADirectory.Observer[] observers = NO_OBSERVERS;

		// note the 8 bytes cost for this flag due to memory padding. Or there are 7
		// more bytes "free" for future fields.
		...
		public int consolidate() {
			...
			synchronized (this.mutex()) {
				...
				count += this.files.keys().removeBy(fileName -> !physicalItems.contains(fileName));
			}
		...
		public int consolidateDirectories() {
			...
			synchronized (this.mutex()) {
				...
				count += this.directories.keys().removeBy(dirName -> !physicalDirectories.contains(dirName));
			}
		...
		public int consolidateFiles() {
			...
			synchronized (this.mutex()) {
				...
				count += this.files.keys().removeBy(fileName -> !physicalFiles.contains(fileName));
			}
		...
		public final boolean contains(final AItem item) {
			// cannot lock both since hierarchy order is not clear. But one is sufficient,
			// anyway.
			synchronized (this.mutex()) {
		...
		public final AFile ensureFile(final String identifier, final String name, final String type) {
			...
			if (identifier == null) {
				...
				effType = type != null ? type : this.fileSystem().deriveFileType(identifier) // might return null yet
																								// again.
				;
			}
		...
		@Override
		public final ADirectory resolveDirectoryPath(final String[] pathElements, final int offset, final int length) {
			// length means distance in this case. If no more distance remains (length 0),
			// "this" is the result.
			if (length == 0) {
				// note: identifier validation makes no sense at this point. Length 0 always
				// means "this".
				return this;
			}
			...
			// requires the central lock but calls an internal method, so this lock must be
			// acquired here
			synchronized (this.fileSystem()) {
				...
				for (int o = offset, l = length; l > 0; o++, l--) {
					...
					if (resolvedChildDirectory == null) {
						throw new AfsExceptionUnresolvablePathElement(VarString.New()
								.add("Unresolvable path element \"").add(pathElements[offset]).add("\" in path \"")
								.addAll(pathElements, VarString::commaSpace).deleteLast(2).add('"', '.').toString());
					}

					// recursion implemented as iteration instead of recursive calls (potential
					// stack overflow)
		...
		@Override
		public final <R> R accessFiles(final Function<? super XGettingTable<String, ? extends AFile>, R> logic) {
			synchronized (this.mutex()) {
		...
		@Override
		public final <S, R> R accessDirectories(final S subject,
		...
		@Override
		public final <S, R> R accessFiles(final S subject,
		...
		public final boolean removeObserver(final ADirectory.Observer observer) {
			synchronized (this.mutex()) {
				// best performance and special (also weirdly common) case for last/sole
				// observer.
	...
	public static ADirectory New(final ADirectory parent, final String identifier) {
		return new ADirectory.Default(notNull(parent), notNull(identifier));
	}
	...
	public static ADirectory actual(final ADirectory directory) {
		return directory instanceof ADirectory.Wrapper ? ((ADirectory.Wrapper) directory).actual() : directory;
	}
