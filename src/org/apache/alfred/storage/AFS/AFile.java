...
package org.apache.alfred.afs.types;
...
import org.apache.alfred.collections.XArrays;
...
public interface AFile extends AItem {
	...
	/**
	 * A simple String representing the "name" of the file. While no two files can
	 * have the same {@link #identifier()} inside a given directory, any number of
	 * files can have the same name.
	 * <p>
	 * Depending on the file system implementation, {@link #name()} might be the
	 * same value as {@link #identifier()}, but while {@link #identifier()} is
	 * guaranteed to be a unique local identifier for any file system,
	 * {@link #name()} is not.
	 * 
	 * @see #toPathString()
	 * @see #identifier()
	 ...
	 * <p>
	 * If such an information makes no sense for a certain file system, this value
	...
	/**
	 * Returns the size in bytes of this file's content, without any space required
	 * for file metadata (name etc.).
	 * 
	...
	public default boolean ensureExists() {
		// exists is very expensive, so double-checking to avoid a lambda instance is a
		// bad idea
		// (20.09.2020 TM) priv#392
		// if(this.exists())
		// {
	...
	public static AFile New(final ADirectory parent, final String identifier) {
		return new AFile.Default(notNull(parent), notNull(identifier));
	}

	public class Default extends AItem.Abstract implements AFile {
		///////////////////////////////////////////////////////////////////////////
		// static fields //
		//////////////////
	...
	// (27.05.2020 TM)TODO: priv#49: Composite Observer implementation with
	// lambda-based factory
	public interface Observer {
		public default void onBeforeFileWrite(final AWritableFile targetFile,
				final Iterable<? extends ByteBuffer> sources) {
			// no-op by default
		}

		public default void onAfterFileWrite(final AWritableFile targetFile,
				final Iterable<? extends ByteBuffer> sources, final long writeCount) {
			// no-op by default
		}

		public default void onBeforeFileMove(final AWritableFile sourceFile, final AWritableFile targetFile) {
			// no-op by default
		}

		public default void onAfterFileMove(final AWritableFile sourceFile, final AWritableFile targetFile) {
			// no-op by default
		}
		...
		public default void onAfterFileDelete(final AWritableFile deletedFile, final boolean result) {
			// no-op by default
		}
	...
	public static AFile actual(final AFile file) {
		return file instanceof AFile.Wrapper ? ((AFile.Wrapper) file).actual() : file;
	}
	...
	public interface Wrapper extends AFile, AItem.Wrapper {
		...
		public abstract class Abstract<U> extends AItem.Base implements AFile.Wrapper {
			...
			@Override
			public AFile actual() {
				return this.actual instanceof AFile.Wrapper ? ((AFile.Wrapper) this.actual).actual() : this.actual;
			}
