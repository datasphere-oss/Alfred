...
import org.apache.alfred.storage.hf.tickdb.pub.mon.NotificationHandler;
import org.apache.alfred.storage.hf.tickdb.pub.mon.PropertyMonitor;
import org.apache.alfred.storage.hf.tickdb.pub.mon.PropertyMonitorHandler;
...
public class RAMDisk implements PropertyMonitorHandler {
    ...
    public enum Properties {
        cacheSize, usedCacheSize, numPages, numOpenedFiles, bytesWritten, bytesRead, queueLength, writerState, failures
    }

    /**
     * The size of data cached on a single page. Normally 8K.
     */
    public static final int PAGE_SIZE = DataCache.PAGE_SIZE;
     ...
     */
    public static final long PAGE_SIZE_WITH_OVERHEAD = DataCache.PAGE_SIZE_WITH_OVERHEAD;
    ...
    public static RAMDisk createCached(int maxNumOpenFiles, long memorySize, long preallocateSize) {
        if (memorySize < DataCache.PAGE_SIZE_WITH_OVERHEAD)
            throw new IllegalArgumentException(
                    "Cache size too small: " + memorySize + "; minimum: " + DataCache.PAGE_SIZE_WITH_OVERHEAD);
        ...
        if (memorySize > limit) {
            RAMDisk.LOGGER.warning("Limiting specified max RAMDisk size (" + memorySize + ") to 0.75 of max memory ("
                    + maxMem + "), i.e. down to " + limit);
    ...
    public static RAMDisk createCacheByNumPages(int maxNumOpenFiles, long numPages, long initialNumPages) {
