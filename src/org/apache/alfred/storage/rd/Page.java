...
package org.apache.alfred.storage.rd;

import org.apache.alfred.util.collections.QuickList;
import org.apache.alfred.util.memory.MemorySizeEstimator;
import org.apache.alfred.util.time.TimeKeeper;
...
final class Page extends QuickList.Entry<Page> {
    ...
    static final long OVERHEAD = MemorySizeEstimator.ARRAY_OVERHEAD + MemorySizeEstimator.OBJECT_OVERHEAD
    ...
    /**
     * Value for {link #startAddress} meaning that this page is not mapped. Also, in
     * unmapped pages {link #fd} == null.
     */
    static final long UNMAPPED = -1;

    /**
     * Value for {link #index} meaning that this page is not mapped. Also, in
     * unmapped pages {link #fd} == null.
     */
     ...
     * initializes the state to this value; the page must be linked to the free pool
     * by the caller.
     */
    ...
    /**
     * Dirty page, which was given out to the writer thread for saving. It will be
     * saved momentarily and become CLEAN, unless it is written to, and then it
     * transitions back to DIRTY. Pages in SAVING state are already removed from the
     * dirty range.
     */
    ...
    private int getOffsetForRead(long address) {
        ...
        assert longOffset >= 0 && longOffset < validLength : this + " is wrong for " + address;
    ...
    int read(long srcAddress, byte[] dest, int destOffset, int length) {
        assert state != FREE : this + " is unmapped";
        assert fd != null;
    ...
    void write(long destAddress, byte b) {
        assert state == WRITING || state == DIRTY : this + " is not writable (WRITING | DIRTY)";
    ...
    int write(long destAddress, byte[] src, int srcOffset, int length) {
        assert state == WRITING || state == DIRTY : this + " is not writable (WRITING | DIRTY)";
     ...
     */
    long getEndAddress() {
        assert startAddress != UNMAPPED && validLength != UNLOADED : this + " is unmapped";
    ...
    //
    /**
     * Map a free page for checkout. The page is unlinked from the free list and
     * mapped to the new owner fd.
     *
     * @param fd          New owner fd.
     * @param address     File address.
     * @param targetState Target state, either {@link #READING} or {@link #WRITING}.
     */
    void freeToCheckout(FD fd, long address, byte targetState) {
        assert targetState == READING || targetState == WRITING : "Illegal target state: " + getStateName(targetState);

        assert state == FREE || state == NEW : this + " is not FREE or NEW";

        if (DEBUG_STATE_TRANSITIONS)
            System.out.println("PAGE STATE:    freeToCheckout: " + this + " -> " + getStateName(targetState)
    ...
    /**
     * Take over a clean page and re-map it to a different fd for checkout. The page
     * is removed from the previous owner's index, unlinked from the clean list,
     * marked as unloaded and mapped to the new owner fd.
     *
     * @param fd          New owner fd.
     * @param address     File address.
     * @param targetState Target state, either {@link #READING} or {@link #WRITING}.
     */
    void takeover(FD fd, long address, byte targetState) {
        assert targetState == READING || targetState == WRITING : "Illegal target state: " + getStateName(targetState);
        ...
        if (DEBUG_STATE_TRANSITIONS)
            System.out.println(
                    "PAGE STATE:    cleanToCheckout: " + this + " -> CLEAN; mapped to " + fd + " at " + address);
    ...
    /**
     * Mark a page as dirty. This method is called before a synchronized write. If
     * the page is dirty, nothing is done. If the page is clean, it is unlinked from
     * the clean list, and marked as DIRTY (this includes its being registered in
     * the dirty page range).
     */
    void mappedToDirty() {
        if (state == DIRTY)
            return;

        assert state == CLEAN || state == SAVING : this + ": expected DIRTY | SAVING";

        if (DEBUG_STATE_TRANSITIONS)
            System.out.println("PAGE STATE:    mappedToDirty: " + this + " -> DIRTY");
    ...
    /**
     * Check in a page that was checked out for load. If the page was checked out
     * for read-write, it is now marked as DIRTY (this includes its being registered
     * in the dirty page range). Otherwise, the page is marked CLEAN. The caller
     * (DataCache) must take care of linking this page to the clean list and
     * notifying threads waiting for clean pages.
     *
     * @return Whether the page is clean
     */
    boolean checkIn() {
        assert state == READING || state == WRITING : this + " is not checked out (" + state + ")";

        if (state == READING) {
            if (DEBUG_STATE_TRANSITIONS)
                System.out.println("PAGE STATE:    checkIn: " + this + " -> CLEAN");
            ...
            if (DEBUG_STATE_TRANSITIONS)
                System.out.println("PAGE STATE:    checkIn: " + this + " -> DIRTY");
    ...
    /**
     * Free a mapped page. This method is typically called when a file is closed or
     * truncated. The page is marked as unloaded and, if CLEAN, is unlinked from the
     * clean list. This method does not take care of updating the dirty page range,
     * because it is typically more efficient to do that once for all pages (on
     * truncate).
     */
    void mappedToFree() {
        if (DEBUG_STATE_TRANSITIONS)
    ...
    /**
     * Transitions a DIRTY page to SAVING, simultaneously copying the data into the
     * supplied job object.
     *
    ...
    void dirtyToSaving(SavePageJob job) {
        ...
        if (DEBUG_STATE_TRANSITIONS)
            System.out.println("PAGE STATE:    dirtyToSaving: " + this + " -> SAVING");
    ...
    /**
     * If the page is in SAVING state, transition it to CLEAN and return true. In
     * this case, the caller must link the page to the clean list and notify threads
     * waiting for clean pages. The page might also be in DIRTY mode, if it were
     * written to at the same time as it was being saved. In this case, return
     * false. The caller should do nothing in the latter case.
     *
     * @return Whether the page is clean.
     */
    ...
    private void map(FD fd, long address) {
        long h = fd.getPrivateHeaderLength();

        assert address >= h : "Address " + address + " is in the private header range (" + h + ") for " + fd;

        assert address < DataCache.MAX_FILE_LENGTH : "Address " + address + " is too large (max "
                + DataCache.MAX_FILE_LENGTH + ")";
     ...
     * @throws IOException
     */
    void load(FD checkfd, long logicalLength) throws IOException {
        assert fd == checkfd : this + " is not mapped to " + checkfd;
        assert validLength == UNLOADED : this + " is already loaded";
        assert state == READING || state == WRITING : this + " is not checked out";
        ...
        validLength = available < DataCache.PAGE_SIZE ? (int) available : DataCache.PAGE_SIZE;
        ...
        assert numRead == validLength : "Failed to load " + validLength + " bytes from " + fd + " at " + startAddress
                + ". Loaded " + numRead + " bytes";
    }
