...
/**
 * Access to this class must be synchronized by the caller, as is the case with
 * I/O objects in most frameworks.
 */
public class FD implements Disposable {
    private static final boolean DEBUG_SAVE = false;

    public static final GrowthPolicy DEFAULT_GROWTH_POLICY = new MixedGrowthPolicy(2, 1 << 10, 1 << 24);
    ...
    /**
     * Field for asserting that two RAFs are never opened for the same FD. Managed
     * and guarded by RAFCache. Do not use directly.
     */
    ...
    public void warmUp() throws IOException, NoFreePagesException {
        assertOpen();
    ...
    /**
     * Called asynchronously by the framework when file is committed. Any exceptions
     * thrown from this method will be logged, but otherwise ignored. The default
     * implementation does nothing.
     */
    protected void onCleanCommit(long cleanLength) throws IOException {
        if (DEBUG_SAVE) {
            System.out.println("COMMIT " + this + "; clean length: " + cleanLength);
        }
    }

    /**
     * Override to provide alternative method of persisting the logical length. The
     * default implementation calls {@link #trimToSize}.
     */
    ...
    /**
     * Override to provide alternative method of persisting the logical length. The
     * default implementation sets logical length to the physical length.
     */
    ...
    /**
     * Override to provide alternative method of formatting the file. The default
     * implementation sets logical length to the result of
    ...
    /**
     * Override to provide additional handling after file truncation; for example,
     * the subclass may want to fully commit the new length. The default
     * implementation does nothing.
     */
    ...
    /**
     * Override to reserve private space at the beginning of the file. The default
    ...
    protected final int directRead(long srcOffset) throws IOException {
        assertOpen();
    ...
    protected final int directRead(long srcOffset, byte[] dest, int destOffset, int length) throws IOException {
        assertOpen();
     ...
     */
    private final void applyGrowthPolicy(RandomAccessFile raf, long minLength) throws IOException {
        ...
        if (growthPolicy != null && minLength > physicalLength) {
            if (physicalLength == PHYSICAL_LENGTH_UNKNOWN) {
                ...
                assert physicalLength == raf.length() : "Cached length " + physicalLength + " does not match actual "
                        + raf.length();
    ...
    protected final void directWrite(long destOffset, byte[] bytes, int srcOffset, int length) throws IOException {
        assertOpen();
    ...
    protected final void directWrite(long destOffset, int b) throws IOException {
        assertOpen();
    ...
    public final int read(long srcAddress, byte[] dest, int destOffset, int length) throws IOException {
        if (length == 0)
            return (0);
    ...
    public final void write(long destAddress, byte b) throws IOException {
        if (dataCache == null) {
    ...
    public final void write(long destAddress, byte[] src, int srcOffset, int length) throws IOException {
        if (length == 0)
            return;
     ...
     */
    public final void waitForFlush() throws IOException, InterruptedException {
        assertOpen();
    ...
    public final void truncate(long length) throws IOException {
        ...
        if (logicalLength < length)
            throw new IllegalArgumentException(
                    "length: " + length + " is greater than current logical length: " + logicalLength);
