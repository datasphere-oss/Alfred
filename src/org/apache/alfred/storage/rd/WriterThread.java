...
import org.apache.alfred.util.runtime.*;
    ...
    public void run() {
        ...
        try {
            while (!stop) {
                ...
                try {
                    // System.out.println (fd + ": directWrite (pos=" + address + ";length=" +
                    // length + ")");
                    fd.directWrite(address, job.data, 0, length);
                } catch (Throwable x) {
                    RAMDisk.LOGGER.log(Level.SEVERE, "Error saving " + job.length + " bytes at " + job.address + " to "
                            + fd + ";\nERROR IGNORED, BUT DATA NOT SAVED.", x);
                    //
                ...
                if (fd.getAutoCommit() && job.isLastDirtyPage && job.followsCleanRange) {
                    try {
                        ...
                        RAMDisk.LOGGER.log(Level.SEVERE, "Error in onCommit () in " + fd + "; IGNORED.", x);
            ...
            RAMDisk.LOGGER.log(Level.SEVERE, "CRITICAL ERROR IN WriterThread. Timebase will shutdown.", x);
