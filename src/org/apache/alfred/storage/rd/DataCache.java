...
import org.apache.alfred.storage.tickdb.pub.mon.NotificationHandler;
import org.apache.alfred.util.collections.QuickList;
import org.apache.alfred.util.collections.generated.LongToObjectHashMap;
...
import static org.apache.alfred.ramdisk.RAMDisk.Properties;
...
final class DataCache {
    ...
    public static final long MAX_FILE_LENGTH = ((long) Integer.MAX_VALUE) << PAGE_SIZE_LOG2;
    ...
    static int getLastPageIndex(FD fd) {
        ...
        assert len <= DataCache.MAX_FILE_LENGTH : fd + " has too high logical length " + len + " (max "
                + DataCache.MAX_FILE_LENGTH + ")";
        ...
        assert n > 0 : fd + ": logical length " + len + " is shorter than private header length " + h;

        return ((int) ((n - 1) >> PAGE_SIZE_LOG2));
    }

    private Page getFreePage(FD fd, long address, byte targetState) {
    ...
    private Page checkOutPageForTakeover(FD fd, long address, byte targetState) throws InterruptedException {
    ...
    private static void assertPageMapped(FD fd, long srcAddress, Page p) {
        ...
        if (srcAddress > fd.getPrivateHeaderLength()) {
            ...
            assert p.getStartAddress() <= srcAddress && p.getEndAddress() >= srcAddress : p + " is wrong for address "
                    + srcAddress;
        }
    }

    public synchronized byte read(FD fd, long srcAddress, PageContainer c) throws InterruptedException {
    ...
    public synchronized int read(FD fd, long srcAddress, byte[] dest, int destOffset, int length, PageContainer c)
    ...
    public synchronized void write(FD fd, long destAddress, byte b, PageContainer c) throws InterruptedException {
    ...
    public synchronized int write(FD fd, long destAddress, byte[] src, int srcOffset, int length, PageContainer c)
            throws InterruptedException {
        assert writer != null : "Data Cache not started";
        ...
        if (page != null) {
            ...
            int ret = page.write(destAddress, src, srcOffset, length);
    ...
    synchronized Page checkOutPageForWarmUp(FD fd, Page previousPage) throws NoFreePagesException {
        final PageIndex pi = fd.pageIndex;
        final long length = fd.getLogicalLength();
        long address = previousPage == null ? fd.getPrivateHeaderLength() : previousPage.getEndAddress();
    ...
    synchronized void beingClosed(FD fd, boolean checkSave) {
        ...
        if (hadUnsavedData && RAMDisk.LOGGER.isLoggable(Level.WARNING))
            RAMDisk.LOGGER.log(Level.WARNING, fd.getFile() + " was closed with unsaved data",
    ...
    synchronized void waitForFlush(FD fd) throws IOException, InterruptedException {
    ...
    synchronized long getNextJobTime(SavePageJob job) {
        ...
        long jobTime = job.fd != null && job.fd.pageIndex != null ? job.fd.pageIndex.modified : Long.MAX_VALUE;
    ...
    synchronized boolean getNextJob(SavePageJob job) {
        for (;;) {
            ...
            if (index == null) {
                ...
                assert dirtyFiles.isEmpty() : "curpi == null but !dirtyFiles.isEmpty ()";
    ...
    // assert state == Page.FREE || state == Page.NEW : page + " is NOT FREE or
    // NEW";
    // page = page.next();
    // }
    //
