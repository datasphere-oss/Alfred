...
import org.apache.alfred.storage.TFS.codecs.*;
import org.apache.alfred.storage.TFS.dataacc.*;
import org.apache.alfred.storage.TFS.pub.AbstractSingleEntityFilter;
import org.apache.alfred.storage.TFS.pub.EntityFilter;
import org.apache.alfred.storage.TFS.pub.ListEntityFilter;
import org.apache.alfred.storage.TFS.pub.SingleEntityFilter;
import org.apache.alfred.storage.TFS.blocks.ObjectPool;
import org.apache.alfred.util.collections.ByteArray;
import org.apache.alfred.util.collections.generated.ByteArrayList;
...
import org.apache.alfred.util.collections.generated.ObjectHashSet;

import java.io.*;

import static org.apache.alfred.storage.TFS.TSFState.*;

/**
 * Implementation of a time slice file. All variables of this class are guarded
 * by "this".
 */
final class TSFile extends TSFolderEntry implements TimeSlice {
    ...
    private static int computeIndexBlockSize(boolean compressed, int numEntities, int formatVersion) {
        return (numEntities * (compressed ? 28 : DataBlockStub.SIZE_ON_DISK)
                + (formatVersion >= 3 ? FILE_HEADER_SIZE : 10));
    }

    private static final int INITIAL_SIZE = FILE_HEADER_SIZE;

    private enum ExpansionStrategy {
        MOVE_BACKWARD, MOVE_FORWARD, SPLIT
    }

    private TSFState state = null;

    // Indicates that file belongs to the write queue. Guarded by PDSImpl.
    // Added to resolve race condition when file processed by WriterThread and does
    // not belongs to queue.

    volatile boolean queued;

    /**
     * Does this file exists on disk. Guarded by root structure lock: it is checked
     * only by the holder of an exclusive structure lock, and modified only by the
     * holder of a shared structure lock.
     */
        ...
        synchronized (this) {
            if (checkouts == null || checkouts.isEmpty()) {
                ...
                if (state == TSFState.DIRTY_CHECKED_OUT) {
                    // PDSImpl.LOGGER.log (Level.FINE, this + ": " + state + "->" +
                    // TSFState.DIRTY_QUEUED_FOR_WRITE);

                    state = DIRTY_QUEUED_FOR_WRITE;
                    addToWriteQueue(cache);

                } else if (state == DIRTY_QUEUED_FOR_WRITE) {
                    LOGGER.warn().append(this).append(": ").append(state).append(" -> ").append(TSFState.CLEAN_CACHED)
                            .commit();
                } else {
    ...
    boolean hasDataFor(EntityFilter filter) throws IOException {
    ...
    boolean hasDataFor(int entity) throws IOException {
        ensureIndexAndDataLoaded(null, null);
    ...
    @Override
    public void insertNotify(EntityFilter loadHint, DAPrivate accessor, long timestamp, int addlLength)
    ...
    synchronized void activate() {
        ...
        // PDSImpl.LOGGER.log(Level.INFO, this + ": " + state + "->" +
        // TSFState.CLEAN_CACHED);
    ...
    synchronized void deactivate() {
        ...
        if (LOGGER.isDebugEnabled())
            LOGGER.debug().append(this).append(": ").append(state).append("-> null").commit(); // append(new
                                                                                               // Exception()).commit();
    ...
    synchronized void checkOutTo(DAPrivate accessor) {
        ...
        if (!check)
            throw new IllegalArgumentException(this + " is ALREADY checked out to " + accessor);

        // call event before creating snapshot, because events will fire, if snapshot
        // contains accessor
        ...
        switch (state) {
            ...
            case DIRTY_QUEUED_FOR_WRITE:
                ...
                // else {
                // PDSImpl.LOGGER.log(Level.WARN, this + ": " + state + "->" +
                // TSFState.DIRTY_CHECKED_OUT + (dropped ? "(deleted)" : ""));
                // }
    ...
    private synchronized boolean storeInternal(BlockCompressor compressor) throws IOException {
        ...
        if (compressedOnDisk) {
            ...
            for (int ii = 0; ii < numEntities; ii++) {
                ...
                if (db.getDataLength() == 0)
                    ...
                    compLengths[ii] = compressor.deflate(data.getArray(), data.getOffset(), db.getDataLength(),
                            compressedData);
                }
            }
    ...
    synchronized boolean checkedInBy(DAPrivate accessor) {
        ...
        if (!check)
            throw new IllegalArgumentException(this + " is not checked out to " + accessor);
    ...
    private synchronized ExpansionStrategy checkInsert(long timestamp, int addlLength) {
    ...
    private void openOrSeek(FileInput in, int toOffset) throws IOException {
        if (!in.isOpened()) {
            //
    ...
    private DataBlock processEntity(FileInput in, int pos, DataBlockInfo dbi, BlockProcessor bp) throws IOException {
    ...
    private DataBlock ensureIndexAndDataLoadedCatchIOX(EntityFilter filter, BlockProcessor bp) {
        try {
    ...
    private DataBlock ensureIndexAndDataLoaded(EntityFilter filter, BlockProcessor bp) throws IOException {
    ...
    private synchronized void readIndexBlock(FileInput input) throws IOException {
        dbs = new ObjectArrayList<>();
        lastTimestamp = Long.MIN_VALUE;
        ...
        for (int ii = 0; ii < numEntities; ii++) {
            ...
            if (prevStub != null && entity <= prevStub.getEntity())
                throw new IOException("Wrong entity ordering: " + entity + " after " + prevStub.getEntity());
            ...
            DataBlockStub dbx = new DataBlockStub(entity, blockOffsetOnDisk, lengthOnDisk, dataLength, startTime,
                    endTime);
    ...
    private synchronized void assertCheckedOutTo(DAPrivate accessor) {
        ...
        assert isCheckedOutTo(accessor) : this + " is not checked out to " + accessor;
    }
    ...
    // latterBlock.setData (formerBlock.getBytes (), splitOffset,
    // latterBlockLength);
    // }
    //
