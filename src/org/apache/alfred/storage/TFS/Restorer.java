...
import org.apache.alfred.storage.tfs.pub.FSUtils;
import org.apache.alfred.storage.tfs.codecs.TSNames;
import org.apache.alfred.storage.tfs.impl.IndexInfo.ChildInfo;
import org.apache.alfred.storage.tfs.pub.TSRoot;
import org.apache.alfred.storage.tfs.raw.MutableRawTSF;
import org.apache.alfred.storage.tfs.raw.DiagPrinter;
import org.apache.alfred.storage.tfs.raw.RawFolder;
import org.apache.alfred.storage.tfs.raw.RawTSF;

import static org.apache.alfred.storage.tfs.PDSImpl.LOGGER;
...
public class Restorer {
    ...
    // Logger.getLogger("deltix.qsrv.dtb.store.impl.Restorer");
    ...
    /**
     * Indicates that path is not contains any actual data. (It may contain empty
     * index file only)
     * 
    ...
    private void restoreTempFolders(AbstractPath entry, boolean isFirst) throws IOException {
        ...
        if (indexInfo != null && hasUnsaved) {
            ...
            Stream.of(items).filter(x -> x.startsWith(TSNames.TMP_PREFIX)).forEach(x -> {
    ...
    private boolean checkReadonly(AbstractPath entry) {
        if (isReadOnly)
            LOGGER.warn("index for %s is not valid. Please turn off ReadOnly mode and restart TimeBase Server.")
                    .with(entry.getPathString());
        return isReadOnly;
    }
    ...
    private void restoreConsistency(IndexInfo indexInfo, IndexInfo tmpIndexInfo, AbstractPath entry, boolean isFirst)
            throws IOException {
        restoreFolderConsistency(indexInfo, entry);
    ...
    private IndexInfo restoreIndex(IndexInfo index, AbstractPath path, boolean isFirst, List<AbstractPath> skipped)
            throws IOException {
        ...
        if (actual != null) {
            ...
            // saved index maybe ahead of current index (not contains deleted files or not
            // saved files)
    ...
    private IndexInfo rebuildIndex(AbstractPath path, boolean isFirst, ArrayList<AbstractPath> skipped)
            throws IOException {
    ...
    private ChildInfo getFolderInfo(String name, AbstractPath parentPath) {
        ...
        if (subIndex != null && subIndex.hasChildren())
            return new ChildInfo(false, TSNames.getEID(name), subIndex.getChildrenInfo().get(0).ts,
                    subIndex.getVersion());
    ...
    private void synchronizeIndexWithFolder(IndexInfo indexInfo, AbstractPath path, boolean isFirst)
            throws IOException {
        ...
        for (int i = 0; i < children.size(); ++i) {
            ...
            if (!curPath.exists()) {
                ...
                if ((time != curChild.ts) || (!firstChild && time == Long.MIN_VALUE)
                        || (firstChild && time != Long.MIN_VALUE)) {
                    if (time != curChild.ts) {
    ...
    // private void clearFolder(List<ChildInfo> children, AbstractPath path) throws
    // IOException {
    ...
    // if (files[i].equals(TSNames.INDEX_NAME) ||
    // files[i].equals(TSNames.ROOT_PROPS_NAME) ||
    // (files[i].equals(TSNames.SYM_REGISTRY_NAME))) {
    // continue;
    // }
    // needRemove = true;
    ...
    private void restoreFolderConsistency(IndexInfo indexInfo, AbstractPath entry) throws IOException {
        ...
        for (int i = 0; i < children.size(); ++i) {
            ...
            if (!curPath.exists()) {
                ...
                if (filePath != null) {
                    LOGGER.info("Entry '" + filePath.getName() + "' is moved from '" + filePath.getParentPath()
                            + "' to '" + curPath.getParentPath() + "'.");
                ...
                AbstractPath tmpPath = curPath.getFileSystem().createPath(curPath.getParentPath(),
                        TSNames.TMP_PREFIX + curPath.getName());
                if (tmpPath.exists()) {
    ...
    private void moveFromTemp(AbstractPath tmpPath, AbstractPath curPath) throws IOException {
        ...
        for (int ii = 0; ii < files.length; ii++) {
            if (!files[ii].contains("index")) {
                ...
                LOGGER.warn("File '" + tmpFilePath.getName() + "' is moved from '" + tmpFilePath.getParentPath()
                        + "' to '" + tmpFilePath.getParentPath().getPathString() + "'.");
                count++;
            }
