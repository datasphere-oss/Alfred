...
import org.apache.alfred.storage.tfs.codecs.BlockCompressor;
            ...
            for (;;) {
                ...
                try {
                    ...
                    if (tsf.getState() == TSFState.DIRTY_CHECKED_OUT) {
                        ...
                        LOGGER.warn().append("Storing ").append(tsf).append(" with wrong state: ")
                                .append(tsf.getState()).commit();
                    }
                    ...
                    if (tsf.isDropped()) {
                        if (logThisOne)
                            LOGGER.debug().append("Dropping ").append(tsf.getPath().getPathString()).append(" ...")
                                    .commit();
                        ...
                        if (tsf.store(compressor)) {
                            ...
                            if (logThisOne)
                                LOGGER.debug().append(tsf).append(" was stored [").append(tsf.getState()).append("]")
                                        .commit();
                        } else {
                            if (logThisOne)
                                LOGGER.debug().append(tsf).append(" wasn't stored [").append(tsf.getState()).append("]")
                                        .commit();
                        }
