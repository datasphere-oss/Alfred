...
import org.apache.alfred.gflog.api.AppendableEntry;
import org.apache.alfred.gflog.api.Log;
import org.apache.alfred.gflog.api.LogLevel;
import org.apache.alfred.gflog.api.Loggable;
import org.apache.alfred.qsrv.dtb.fs.pub.AbstractPath;
import org.apache.alfred.util.lang.MathUtil;
 ...
 *
 */
abstract class TSFolderEntry implements Comparable<TSFolderEntry>, Loggable {
    ...
    TSFolderEntry(TSFolder parent, int id, long startTimestamp) {
    ...
    private void logTrace() {
        ...
        // usages.add(useCount + " - " + Arrays.toString(new
    ...
    final void assertNotLockedByCurrentThread() {
        assert !Thread.holdsLock(this) : this + " is already locked by " + Thread.currentThread()
    ...
    /**
     * Must be public because the TimeSlice interface implemented by TSFile wants it
     * public.
     */
    ...
    public void appendTo(AppendableEntry entry) {
        if (useCount == DESTROYED) {
            ...
            entry.append(path).append(" @").append(startTimestamp); // Do not obtain lock to avoid nasty logger
                                                                    // recursion
        } else if (root.tryAcquireSharedLock()) {
            try {
                String path = getPathString();
    ...
    synchronized void destroy(boolean force) {
        if (!force && useCount != 0)
            throw new IllegalStateException(this + " is being destroyed while used; useCount = " + useCount);
