...
import org.apache.alfred.storage.TFS.pub.EntityTimeRange;
import org.apache.alfred.util.io.UncheckedIOException;
...
/**
 * Utility class, containing the implementation of tree operations that span
 * multiple nodes. Methods of this class do not explicitly synchronize on nodes.
 * Nodes contain synchronized methods, but never obtain locks on each other.
 */
abstract class TreeOps {
    ...
    /**
     * Recursively search for the first TSF for reading data at or after the
     * specified timestamp.
     *
     * <p>
     * This method returns a "used" TSF, i.e. it calls
     * {@link TSFolderEntry#useMore()} ()} on the return value, if not null. The
     * caller must {@link TSFolderEntry#useLess()} the returned TSF when it's no
     * longer needed.
     * </p>
     *
     * <p>
     * Calling thread must be holding a structure lock.
     * </p>
     *
     * @return A "used" TSF, or null if none found.
     */
    static TSFile findTSFForRead(TSFolder folder, long timestamp) throws IOException {
        use(folder);

        try {
            TSFolderEntry child = folder.findEntry(timestamp, false);
    ...
    static TSFile findTSFByRef(TSFolder folder, TSRefImpl tsref) throws IOException {
        return (findTSFByPath(folder, tsref.getIdPath(), 0));
    }

    private static TSFile findTSFByPath(TSFolder folder, int[] idPath, int idx) throws IOException {
        use(folder);
        ...
        try {
            ...
            if (child == null)
                throw new IllegalArgumentException(
                        "Child id " + id + " not found in " + folder + " [TEMPORARILY A IllegalArgumentException]");
    ...
    /**
     * Recursively search for the first TSF that is suitable for inserting data at
     * the specified timestamp, possibly creating one.
     *
     * <p>
     * This method returns a "used" TSF, i.e. it calls
     * {@link TSFolderEntry#useMore()} on the return value. The caller must
     * {@link TSFolderEntry#useLess()} the returned TSF when it's no longer needed.
     * </p>
     *
     * <p>
     * Calling thread must be holding an exclusive structure lock.
     * </p>
     *
     * @return A "used" TSF.
     */
    static TSFile findTSFForInsert(TSFolder folder, long timestamp) throws IOException {
        use(folder);
     ...
     * <p>
     * This method returns a "used" TSF, i.e. it calls
     * {@link TreeOps#use(TSFolderEntry)} on the return value, if not null. The
     * caller must {@link TreeOps#unuse(TSFolderEntry)} the returned TSF when it's
     * no longer needed.
     * </p>
     *
     * <p>
     ...
     */
    @Nullable
    static TSFile getNextFile(TSFile tsf, EntityFilter filter) throws IOException {
     ...
     * <p>
     * This method returns a "used" TSF, i.e. it calls
     * {@link TSFolderEntry#useMore()} on the return value, if not null. The caller
     * must {@link TSFolderEntry#useLess()} the returned TSF when it's no longer
     * needed.
     * </p>
     *
     * <p>
     ...
     * @return A "used" TSF, or null if none found.
     */
    static TSFile getPreviousFile(TSFile tsf, EntityFilter filter) throws IOException {
    ...
    static TSFile getFirstFile(TSFolder folder, EntityFilter filter) throws IOException {
        use(folder);
    ...
    static TSFile getLastFile(TSFolder folder, EntityFilter filter) throws IOException {
        use(folder);
    ...
    static void insertNotify(TSFolder f) throws IOException {
        if (f.hasRoomFor(1))
            return;
    ...
    private static void splitIntermediate(TSFolder f) throws IOException {
    ...
    //
    // try {
    // newFolder.setTmpName (TSNames.TMP_PREFIX + f.getName ()); //TODO: use
    // newFolder.getName() instead? Otherwise temp name for child has parent's name
    // newFolder.getPath ().makeFolder ();
    //
    // newFolder.moveChildrenHere (f, firstLatterChild, false);
    //
    ...
    private static void splitRoot(TSRootFolder root) throws IOException {
    ...
    private static TSFolder splitRootPart(TSRootFolder root, int pivotIdx, boolean fromLeftOfPivot) throws IOException {
        long ts = fromLeftOfPivot ? root.getStartTimestamp() : root.getChild(pivotIdx).getStartTimestamp();
    ...
    static AbstractPath makeTempPath(AbstractPath folder, String finalName) {
    ...
    static void finalize(AbstractPath tmp) throws IOException {
        String tmpName = tmp.getName();

        if (!tmpName.startsWith(TSNames.TMP_PREFIX))
            throw new IllegalArgumentException("Not a TMP file: " + tmp);

        AbstractPath actual = tmp.getParentPath().append(tmpName.substring(TSNames.TMP_PREFIX_LENGTH));
    ...
    static void propagateDataAddition(TSFolderEntry entry, int newEntity) throws IOException {
        for (;;) {
    ...
    static void propagateDataDeletion(TSFolderEntry entry, int entity) {
        for (;;) {
            TSFolder parent = entry.getParent();
    ...
    static void getFromTimestamp(TSFolder entry, List<EntityTimeRange> ranges) throws IOException {
    ...
    static void getFromTimestamp(TSFile file, List<EntityTimeRange> ranges) {
        use(file);
    ...
    static void getToTimestamp(TSFolder entry, List<EntityTimeRange> ranges) throws IOException {
        use(entry);
    ...
    static void getToTimestamp(TSFile file, List<EntityTimeRange> ranges) {
        if (file != null)
            use(file);
    ...
    static long getFromTimestamp(TSFolderEntry entry, int entity) throws IOException {
        if (entry == null)
            return (Long.MAX_VALUE);
    ...
    static long getToTimestamp(TSFolderEntry entry, int entity) throws IOException {
        if (entry == null)
