...
import org.apache.alfred.hf.pub.codec.*;
...
public class DataWriterImpl extends BlockAccessorBase implements DataWriter {
    ...
    //
    @Override
    public synchronized void open(long nstime, EntityFilter loadHint) {
    ...
    @Override
    public synchronized void insertMessage(int entity, long nstime, int typeCode, TSMessageProducer producer) {
        assertOpen();
        ...
        for (int attempt = 0;; attempt++) {
            if (attempt == MAX_ATTEMPTS)
                throw new RuntimeException("MAX_ATTEMPTS to insert [" + GMT.formatNanos(nstime) + ", having "
                        + packetLength + " bytes] into " + currentTimeSlice + " denied; aborting.");
    ...
    @Override
    public synchronized void appendMessage(int entity, long nstime, int typeCode, TSMessageProducer producer,
            boolean truncate) {
        assertOpen();
