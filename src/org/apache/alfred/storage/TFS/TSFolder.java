...
import org.apache.alfred.storage.TFS.codecs.TSFFormat;
import org.apache.alfred.storage.TFS.codecs.TSNames;
import org.apache.alfred.storage.TFS.pub.AbstractSingleEntityFilter;
import org.apache.alfred.storage.TFS.pub.EntityFilter;
import org.apache.alfred.util.collections.generated.IntegerArrayList;
...
/**
 * Implementation of a time slice folder. The content of the activated folder is
 * guarded by the root structure lock. The activation process itself is guarded
 * by "this".
 */
abstract class TSFolder extends TSFolderEntry {
    ...
    /**
     * Whether data in folder changed between store temp index and finalize it.
     * Guarded by the structure lock.
     */
     ...
     *
     * Basically for each entity (instrument) this list stores the first file and
     * the last data file
     */
    ...
    TSFolder(TSFolder parent, int id, long timestamp) {
     ...
     * @throws IOException
     */
    TSFolderEntry findEntry(long timestamp, boolean create) throws IOException {
        ensureLoaded();
        ...
        if (pos < 0) {
            ...
            if (pos < 0)
                throw new IllegalArgumentException("Looking for out-of-range timestamp " + timestamp + " in " + this);
        }
    ...
    TSFolderEntry getChild(int idx) {
        ...
        if (result.getIdxInParent() != idx)
            throw new IllegalStateException(
                    result + " at idx " + idx + " in " + this + " claims idx = " + result.getIdxInParent());
    ...
    TSFolderEntry getFirstChildWithDataFor(int entity) throws IOException {
        EntityIndexEntry ee = findEntityIndexEntry(entity);

        return (ee == null ? null : ee.first);
    }

    TSFolderEntry getLastChildWithDataFor(int entity) throws IOException {
        EntityIndexEntry ee = findEntityIndexEntry(entity);

        return (ee == null ? null : ee.last);
    }

    TSFolderEntry getFirstChildWithDataFor(EntityFilter filter) throws IOException {
        if (filter == null)
    ...
    boolean isAccepted(TSFolderEntry entry, EntityFilter filter) throws IOException {
        if (filter == null)
            return true;
        ...
        for (int ii = 0; ii < numEntries; ii++) {
            ...
            if (ee.first.getIdxInParent() <= entry.getIdxInParent()
                    && ee.last.getIdxInParent() >= entry.getIdxInParent())
                return true;
        }
    ...
    TSFolderEntry getLastChildWithDataFor(EntityFilter filter) throws IOException {
        if (filter == null)
            return (getLastEntry());
    ...
    final void insertNewChildAfter(TSFolderEntry after, TSFolderEntry newEntry) {
        assert root.currentThreadHoldsWriteLock();

        if (after.getParent() != this)
            throw new IllegalArgumentException(after + " does not have " + this + " as its parent");

        if (newEntry.getParent() != this)
            throw new IllegalArgumentException(newEntry + " does not have " + this + " as its parent");
    ...
    synchronized boolean dataAdded(TSFolderEntry child, int entity) throws IOException {
        ensureLoaded();
    ...
    void dataMovedLeftToRight(TSFolderEntry fromLeft, TSFolderEntry toRight, int entity) throws IOException {
        ensureLoaded();
    ...
    void moveChildrenHere(TSFolder oldParent, int pivotIdx, boolean fromLeftOfPivot) throws IOException {
        assert root.currentThreadHoldsWriteLock();
        ...
        for (int ii = 0; ii < oldEntIdx.size();) {
            ...
            //
            if (fromLeftOfPivot ? later(oldEntry.first, lastChild) : earlier(oldEntry.last, firstChild)) {
                ii++;
                continue;
            }
            //
            ...
            boolean entrySpansFolders = fromLeftOfPivot ? later(oldEntry.last, lastChild)
            ...
            if (!shiftDown) {
                if (entrySpansFolders) {
                    ...
                    if (check)
                        throw new IllegalStateException(myParent + " thinks entity " + entity
                                + " is new, but we just split it from existing child " + oldParent);
                } else
            ...
            if (entrySpansFolders) {
                if (fromLeftOfPivot) {
                    oldEntry.first = shiftDown ? this : oldChildList.get(pivotIdx);

                    elast = lastChild;
                } else {
                    efirst = firstChild;

                    oldEntry.last = shiftDown ? this : oldChildList.get(pivotIdx - 1);
                }
            } else if (shiftDown) {
        ...
        for (int ii = firstIdx; ii < limitIdx; ii++) {
            ...
            if (child.setParent(this)) {
                ...
                if (oldParent.decUseCount())
                    throw new IllegalStateException(oldParent + " got deactivated in the middle of moving children");
    ...
    private EntityIndexEntry findEntityIndexEntry(int entity) throws IOException {
        ensureLoaded();
    ...
    private synchronized void ensureLoaded() throws IOException {
        ...
        try (DataInputStream dis = new DataInputStream(BufferedStreamUtil.wrapWithBuffered(fp.openInput(0)))) {
            int formatVersion = dis.readUnsignedShort();

            if (formatVersion > TSFFormat.INDEX_FORMAT_VERSION)
                throw new IOException(fp + ": Unrecognized format version " + formatVersion);
            ...
            IntegerToObjectHashMap<TSFolderEntry> idToChild = new IntegerToObjectHashMap<>(numChildren);
            ...
            for (int ii = 0; ii < numChildren; ii++) {
                ...
                if (index == 0) {
                    if (ts != getStartTimestamp())
                        throw new IOException("Bad first child (id=" + eid + ") ts in " + path + ": " + ts
                                + " (expected " + getStartTimestamp());
                } else if (isFile && ts == Long.MIN_VALUE) {
                    // just skip empty files
                    LOGGER.warn().append("Skip file: ").append(TSNames.buildFileName(eid)).append(" in path: ")
                            .append(path).commit();
                    continue;
                } else if (ts < prevTS)
                    throw new IOException("Bad child ts order in " + path + ": " + prevTS + " --> " + ts);
            ...
            for (int ii = 0; ii < numEntities; ii++) {
                ...
                if (entity <= prevEntity)
                    throw new IOException("Bad entity order in " + path + ": " + prevEntity + " --> " + entity);
                ...
                if (first == null)
                    throw new IOException("Bad entity index in " + fp + ": child is " + firstId + " was not found");

                int lastId = dis.readUnsignedShort();
                TSFolderEntry last = idToChild.get(lastId, null);

                if (last == null)
                    throw new IOException("Bad entity index in " + fp + ": child is " + lastId + " was not found");

                EntityIndexEntry ee = new EntityIndexEntry(entity, first, last);
    ...
    synchronized boolean storeIndexFile() throws IOException {
        ...
        try (DataOutputStream dos = new DataOutputStream(new BufferedOutputStream(tmp.openOutput(fileSize)))) {
    ...
    synchronized boolean checkConsistent() {
        ...
        if (numChildren == 0) {
            if (numEntities != 0)
                throw new AssertionError(this + " has no children, but non-empty entity index");
        ...
        for (int ii = 0; ii < numChildren; ii++) {
            ...
            if (child.getIdxInParent() != ii)
                throw new AssertionError("child " + child + " #" + ii + " is mistaken about its position: claims "
                        + child.getIdxInParent());

            int cid = child.getId();

            if (idsSeen.get(cid))
                throw new AssertionError("child " + child + " #" + ii + " has duplicate id=" + cid);
            ...
            if (ii == 0) {
                if (cts != getStartTimestamp())
                    throw new AssertionError("First child " + child + " of " + this + " has ts=" + cts
                            + ", different from this ts=" + getStartTimestamp());
            } else {
                ...
                if (cts != Long.MIN_VALUE && cts < pts)
                    throw new AssertionError(
                            "child " + child + " #" + ii + " has ts=" + cts + " out of order with previous ts=" + pts);
            }
        }
        ...
        for (int ii = 0; ii < numEntities; ii++) {
            ...
            if (entity <= prevEntity)
                throw new AssertionError("Entity " + entity + " #" + ii + " was expected to be > " + prevEntity);

            prevEntity = entity;

            if (ee.first == null || ee.last == null)
                throw new AssertionError("Null child pointers in index entry for entity " + entity + " #" + ii);

            int x1 = children.indexOf(ee.first);
            int x2 = children.indexOf(ee.last);

            if (x1 < 0)
                throw new AssertionError("In index entry for entity " + entity + " #" + ii + " left child " + ee.first
                        + " was not found in this folder.");

            if (x2 < 0)
                throw new AssertionError("In index entry for entity " + entity + " #" + ii + " right child " + ee.first
                        + " was not found in this folder.");

            if (x1 > x2)
                throw new AssertionError(
                        "Index entry for entity " + entity + " #" + ii + " lists children in wrong order: " + ee.first
                                + " .. " + ee.last + " (their indexes are " + x1 + " .. " + x2 + ")");
        }
    ...
    // LOGGER.level(Level.WARN).append("The last data file in the folder is missing:
    // ").append(missing.getPathString()).commit();
    ...
    //
    // } else {
    // throw new AssertionError("Missing data file " + missing + " (Run Repair
    // Shop)");
    // }
    // }
    // }
