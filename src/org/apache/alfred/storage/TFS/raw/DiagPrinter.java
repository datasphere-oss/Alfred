...
public class DiagPrinter extends DiagListener {
    ...
    public void badEntityOrder(RawNode node, int prevEntity, int nextIdx, int nextEntity) {
        ...
        format("Entity id %d out of order at position %d, expected > %d\n", nextEntity, nextIdx, prevEntity);
    }
    ...
    public void badFirstChildIdInIndex(RawFolder folder, int atIdx, int firstId) {
        ...
        format("First child %d referenced by index entry at position %d was not found\n", firstId, atIdx);
    }
    ...
    public void badLastChildIdInIndex(RawFolder folder, int atIdx, int lastId) {
        ...
        format("Last child %d referenced by index entry at position %d was not found\n", lastId, atIdx);
    }
    ...
    public void badFolderIndexRange(RawFolder folder, int atIdx, int fidx, int lidx) {
        ...
        format("Index entry at position %d points to children in wrong order: %d .. %d\n", atIdx, fidx, lidx);
    }
    ...
    public void fileTooShortForIndex(RawTSF tsf, int blockOffset, long physicalLength) {
        ...
        format("Physical length %,d is insufficient even for the index block (size=%,d)\n", physicalLength,
                blockOffset);
    }
    ...
    public void fileTooShortForBlock(RawTSF tsf, RawDataBlock rdb, long physicalLength) {
        ...
        format("Physical length %,d is insufficient at block #%,d\n", physicalLength, rdb.getIdxInFile());
    }
    ...
    public void badFileIndexRange(RawTSF tsf, RawDataBlock rdb) {
        ...
        format("Block #%,d has timestamps reversed: %d .. %d\n", rdb.getIdxInFile(), rdb.getFirstTimestamp(),
    ...
    public void fileLengthDiscrepancy(RawTSF tsf, int blockOffset, long physicalLength) {
        ...
        format("Computed file length %,d differs from phystical length %,d\n", blockOffset, physicalLength);
    }
    ...
    public void unindexedEntity(RawFolder folder, RawFolderEntry child, int entity) {
        ...
        format("Child #%d '%s' has data for unindexed entity %d\n", child.getIdxInParent(), child.getName(), entity);
    }
    ...
    public void entityOutOfIndexedRange(RawFolder folder, RawEntityIndexEntry entry, RawFolderEntry child) {
        ...
        format("Child #%d '%s' has data for entity %d\n" + "    This child's index is out of expected range %d .. %d\n",
                child.getIdxInParent(), child.getName(), entry.getEntity(), entry.getFirstChildIdx(),
    ...
    public void indexedEntityNotPresent(RawFolder folder, RawEntityIndexEntry entry) {
        ...
        format("Entity %d, supposedly found in children %d .. %d, was not found at all\n", entry.getEntity(),
                entry.getFirstChildIdx(), entry.getLastChildIdx());
    }

    @Override
    public void entityRangeDiscrepancy(RawFolder folder, RawEntityIndexEntry observed,
            RawEntityIndexEntry readFromIndex) {
        appendPrefix(folder);

        format("Incorrect index for entity %d: expected %d .. %d; observed: %d .. %d\n", observed.getEntity(),
                readFromIndex.getFirstChildIdx(), readFromIndex.getLastChildIdx(), observed.getFirstChildIdx(),
    ...
    public void firstChildHasWrongTimestamp(RawFolder folder, RawFolderEntry rfe, long startTimestamp) {
        ...
        format("First folder entry %s has timestamp %d, different from folder's timestamp %d\n", rfe.getName(),
                rfe.getStartTimestamp(), startTimestamp);
    }
    ...
    public void badChildTimeOrder(RawFolder folder, RawFolderEntry prev, RawFolderEntry rfe) {
        ...
        format("Timestamp order violation: entry #%d (%s) has ts=%d, followed by entry #%d (%s) with ts=%d\n",
                prev.getIdxInParent(), prev.getName(), prev.getStartTimestamp(), rfe.getIdxInParent(), rfe.getName(),
    ...
    public void childTimeBeyondLimit(RawFolder folder, RawFolderEntry rfe, long limitTimestamp) {
        ...
        format("Folder entry #%d (%s) has ts=%d beyond expected limit ts=%d\n", rfe.getIdxInParent(), rfe.getName(),
                rfe.getStartTimestamp(), limitTimestamp);
    }
    ...
    public void blockOutOfTimeRange(RawTSF file, RawDataBlock rdb, long startTimestamp, long limitTimestamp) {
        ...
        format("Block #%d for entity %d has indexed time range:\n" + "    [%d .. %d] out of expected range\n"
                + "    [%d .. %d]\n", rdb.getIdxInFile(), rdb.getEntity(), rdb.getFirstTimestamp(),
                rdb.getLastTimestamp(), startTimestamp, limitTimestamp);
    }
    ...
    public void unreadableTimestamp(RawTSF file, RawDataBlock db, int msgOffset, int msgIdx, Throwable x) {
        ...
        format("In block #%d for entity %d, which starts at offset %d:\n"
                + "    Error reading timestamp in message #%d:\n" + "    %s\n", db.getIdxInFile(), db.getEntity(),
                db.getOffset(), msgIdx, x.toString());
    }
    ...
    public void firstTimestampMismatch(RawTSF file, RawDataBlock db, int msgOffset, int msgIdx, long ts) {
        ...
        format("In block #%d for entity %d, which starts at offset %d:\n"
                ...
                db.getEntity(), db.getOffset(), ts, db.getFirstTimestamp());
    }

    @Override
    public void endOfBlockReadingMessageType(RawTSF file, RawDataBlock db, int msgOffset, int msgIdx, long ts,
            int position) {
        appendPrefix(file);

        format("In block #%d for entity %d, which starts at offset %d:\n" + "    In message #%d at offset %d, ts=%d:\n"
                + "    End of block while reading the message type field\n", db.getIdxInFile(), db.getEntity(),
                db.getOffset(), msgIdx, msgOffset, ts);
    }

    @Override
    public void unreadableBodySize(RawTSF file, RawDataBlock db, int msgOffset, int msgIdx, long ts, boolean inHead,
            int sizeOffset, Throwable x) {
        appendPrefix(file);

        format("In block #%d for entity %d, which starts at offset %d:\n" + "    In message #%d at offset %d, ts=%d:\n"
                + "    Error reading the message size field in %s:\n" + "    %s", db.getIdxInFile(), db.getEntity(),
                db.getOffset(), msgIdx, msgOffset, ts, inHead ? "head" : "tail", x.toString());
    }

    @Override
    public void tailSizeMismatch(RawTSF file, RawDataBlock db, int msgOffset, int msgIdx, long ts, int tailSizeOffset,
            int tailSize) {
        appendPrefix(file);

        format("In block #%d for entity %d, which starts at offset %d:\n" + "    In message #%d at offset %d, ts=%d:\n"
                + "    Tail size=%d at offset %d (should have been equal)\n", db.getIdxInFile(), db.getEntity(),
                db.getOffset(), msgIdx, msgOffset, ts, tailSize, tailSizeOffset);
    }

    @Override
    public void endOfBlockReadingBody(RawTSF file, RawDataBlock db, int msgOffset, int msgIdx, long ts, int bodyOffset,
            int size) {
        appendPrefix(file);

        format("In block #%d for entity %d, which starts at offset %d:\n" + "    In message #%d at offset %d, ts=%d:\n"
                + "    End of block reading a %d byte body at offset %d\n", db.getIdxInFile(), db.getEntity(),
                db.getOffset(), msgIdx, msgOffset, ts, size, bodyOffset);
    }
    ...
    public void lastTimestampMismatch(RawTSF file, RawDataBlock db, int msgOffset, int msgIdx, long ts) {
        ...
        format("In block #%d for entity %d, which starts at offset %d:\n" + "    In last message #%d at offset %d:\n"
                ...
                db.getOffset(), msgIdx, msgOffset, ts, db.getLastTimestamp());
    }

    @Override
    public void messageOutOfOrder(RawTSF file, RawDataBlock db, int msgOffset, int msgIdx, long prevTimestamp,
            long ts) {
        appendPrefix(file);

        format("In block #%d for entity %d, which starts at offset %d:\n" + "    In message #%d at offset %d:\n"
                + "    Timestamp %d is less than previous timestamp %d\n", db.getIdxInFile(), db.getEntity(),
                db.getOffset(), msgIdx, msgOffset, ts, prevTimestamp);
    }
}
