...
import org.apache.alfred.storage.codecs.*;
import org.apache.alfred.storage.pub.*;
import org.apache.alfred.storage.pub.codec.TimeCodec;
...
public class RawTSF extends RawNode {
    ...
    @Override
    public void readIndex(long startTimestamp, long limitTimestamp, DiagListener dlnr) throws IOException {
        this.dlnr = dlnr;
    ...
    protected void readIndex(InputStream is, long startTimestamp, long limitTimestamp) throws IOException {
        ...
        for (int ii = 0; ii < numEntities; ii++) {
            ...
            RawDataBlock rdb = new RawDataBlock(ii, entity, blockOffset, dataLength, lengthOnDisk, firstTimestamp,
                    lastTimestamp);
    ...
    public void verify(DiagListener dlnr, long startTimestamp, long limitTimestamp) throws IOException {
        this.dlnr = dlnr;
    ...
    protected void read(long startTimestamp, long limitTimestamp, boolean isVerify) throws IOException {
        ...
        try (BufferedInputStream is = open()) {
            ...
            BlockDecompressor decomp = isCompressed() ? BlockCompressorFactory.createDecompressor(algorithm) : null;
     ...
     */
    protected boolean verifyBlock(MemoryDataInput mdi, RawDataBlock db) {
        ...
        for (;;) {
            ...
            if (!mdi.hasAvail()) {
                dlnr.endOfBlockReadingMessageType(this, db, msgOffset, msgIdx, ts, mdi.getPosition() - msgOffset);
                return (false);
            }
            ...
            try {
                ...
                dlnr.unreadableBodySize(this, db, msgOffset, msgIdx, ts, true, sizeOffset, x);
                return (false);
            }
            ...
            if (mdi.getAvail() < size) {
                dlnr.endOfBlockReadingBody(this, db, msgOffset, msgIdx, ts, bodyOffset, size);
            ...
            try {
                ...
                dlnr.unreadableBodySize(this, db, msgOffset, msgIdx, ts, false, tailSizeOffset, x);
                return (false);
            }

            if (tailSizeValue != tailSizeOffset) {
                dlnr.tailSizeMismatch(this, db, msgOffset, msgIdx, ts, tailSizeOffset, tailSizeValue);
                return (false);
            }
