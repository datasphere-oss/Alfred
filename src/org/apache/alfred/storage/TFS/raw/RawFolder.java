...
import org.apache.alfred.storage.codecs.TSFFormat;
...
public class RawFolder extends RawNode {
    private static final Comparator2<RawEntityIndexEntry, Integer> REIE_COMP2 = new Comparator2<RawEntityIndexEntry, Integer>() {
        @Override
    ...
    public void readIndex(long startTimestamp, long limitTimestamp, DiagListener dlnr) throws IOException {
        try (DataInputStream dis = new DataInputStream(new BufferedInputStream(indexPath.openInput(0)))) {
            ...
            for (int ii = 0; ii < numChildren; ii++) {
                ...
                if (ii == 0) {
                    if (startTimestamp != Verifier.TS_UNKNOWN && ts != startTimestamp)
                        dlnr.firstChildHasWrongTimestamp(this, rfe, startTimestamp);
                } else {
    ...
    public void verify(DiagListener dlnr, long startTimestamp, long limitTimestamp) throws IOException {
        ...
        IntegerToObjectHashMap<RawEntityIndexEntry> observedIndex = new IntegerToObjectHashMap<>(numEntities);
        ...
        for (int childIdx = 0; childIdx < numChildren; childIdx++) {
            ...
            node.readIndex(child.getStartTimestamp(),
                    nextIdx == numChildren ? limitTimestamp : getChild(nextIdx).getStartTimestamp(), dlnr);
            ...
            for (int ient = 0; ient < numEntitiesThisNode; ient++) {
                ...
                if (entry.getFirstChildIdx() > childIdx || entry.getLastChildIdx() < childIdx)
