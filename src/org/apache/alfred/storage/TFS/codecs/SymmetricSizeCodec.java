...
public class SymmetricSizeCodec {
    ...
    public static int readBackward(MemoryDataInput mdi, int offset) {
        ...
        final int sz = SymmetricSizeCodec.read(bytes, offset - szfsz);
    ...
    public static int read(byte[] bytes, int firstOffset) {
        ...
        if ((head & 0x40) == 0) {
            ...
            if ((tail & 0xC0) != 0x80)
                throw new IllegalArgumentException("Head-Tail mismatch: " + head + ":" + tail);

            return (head & 0x3F | ((tail & 0x3F) << 6));
        }
        ...
        if ((head & 0x20) == 0) {
            ...
            return (head & 0x1F | ((bytes[firstOffset + 1] & 0xFF) << 5) | ((tail & 0x1F) << 13));
        }
        ...
        return (head & 0x1F | ((bytes[firstOffset + 1] & 0xFF) << 5) | ((bytes[firstOffset + 2] & 0xFF) << 13)
                | ((tail & 0x1F) << 21));
    }
}
