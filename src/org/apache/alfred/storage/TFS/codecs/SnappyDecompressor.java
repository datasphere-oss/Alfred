...
import org.apache.alfred.storage.fs.alloc.ByteArrayHeap;
...
public class SnappyDecompressor extends BlockDecompressor {
    ...
    public void inflate(byte[] src, int srcOffset, int srcLength, byte[] out, int outOffset, int outLength) {
        int infLength = 0;
        try {
        ...
        if (infLength != outLength)
            throw new com.epam.deltix.util.io.UncheckedIOException(
