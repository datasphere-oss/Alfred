...
import org.apache.alfred.qsrv.dtb.fs.pub.AbstractFileSystem;
import org.apache.alfred.qsrv.dtb.store.pub.*;
import org.apache.alfred.util.collections.generated.IntegerEnumeration;
 ...
 *  
 */
public final class DataReaderImpl extends BlockAccessorBase
    ...
    @Override
    public synchronized void open(long timestamp, boolean forward, EntityFilter filter) {
    ...
    @Override
    public synchronized void open(TSRef tsref, long timestamp, boolean movePastTSFEnd, EntityFilter filter) {
        if (currentTimeSlice != null) {
            throw new IllegalStateException("Attempt to repeatedly open DataReader");
        }
        this.forward = true;
    ...
    boolean sliceMatchesLimit(TimeSlice timeSlice) {
        if (limit != Long.MAX_VALUE && limit != Long.MIN_VALUE) {
            if ((forward && timeSlice.getStartTimestamp() >= limit)
                    || (!forward && timeSlice.getLimitTimestamp() > limit)) {
                return false;
            }
        }
        return true;
    ...
    private TimeSlice getNextTimeSliceToRead() throws InterruptedException {
        ...
        try {
            ...
            // Contract: if currentTimeSlice is not null, then it's checked out to the
            // reader.
            ...
            if (!prevSliceReleased) {
                // We release prev slice only after we got next slice because it may require
                // initializing prefetch from the current slice
    ...
    TimeSlice getNextTimeSliceToReadNoPrefetch(TimeSlice currentTimeSlice, DAPrivate accessor, boolean keepPrevCheckout)
            throws InterruptedException {
        return currentTimeSlice.getStore().getNextTimeSliceToRead(accessor, currentTimeSlice, currentFilter, // we can
                                                                                                             // miss
                                                                                                             // updates
                                                                                                             // when
                                                                                                             // filter
    ...
    private void initPrefetcher() {
        ...
        if (LOG.isEnabled(LogLevel.DEBUG)) {
            LOG.log(LogLevel.DEBUG).append("Prefetch enabled: ").append(prefetchEnabled).append(", prefetchSize=")
                    .append(prefetchSizeFromStore).append(")").commit();
        }
        if (prefetchEnabled) {
            if (prefetcher == null)
