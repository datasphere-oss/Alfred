...
import org.apache.alfred.storage.codecs.SymmetricSizeCodec;
import org.apache.alfred.storage.pub.*;
import org.apache.alfred.hf.pub.codec.TimeCodec;
...
public final class AccessorBlockLink {
    ...
    public AccessorBlockLink(BlockAccessorBase accessor, TimeSlice ts, DataBlock block) {
    ...
    /**
     * Seeks offset pointing first message with given time or end of block. If
     * messages with given time is not present, offset will point to the last
     * message with time < nstime Returns last message timestamp
     */
    ...
    /*
     * Search & positioning to middle of data. Returns timestamp of message found in
     * the middle
     */
    ...
    void insertMessage(long nstime, byte[] src, int srcOffset, int length) {
        int msgOffset;
    ...
    /**
     * Returns true if block contains data having timstamps that more then given
     * time
     * 
    ...
    /*
     * Split block to the given time Returns number of freed bytes
     */
    ...
    /*
     * Cuts time range from the block. Returns number of freed bytes
     */
    ...
    /*
     * Truncates to given nanosecond time. Returns number of freed bytes;
     */
        ...
        synchronized (block) {
            ...
            if (nextTimestamp != check)
                throw new IllegalStateException(
                        "Timestamp discrepancy (missed update?): expected=" + nextTimestamp + "; read " + check);
    ...
    private void readMessage(MemoryDataInput mdi, TSMessageConsumer processor) {
        long check = TimeCodec.readNanoTime(mdi);
