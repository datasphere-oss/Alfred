...
import org.apache.alfred.util.concurrent.IntermittentlyAvailableResource;
...
public interface DataReader extends DataAccessor, IntermittentlyAvailableResource {
    /**
     * 
     * Resets this accessor to focus on the first message in the specified TSF at or
     * after the specified timestamp.
     * 
     * @param timeSlice      The time slice to focus on.
     * @param timestamp      Fast forward to specified timestamp. Pass
     *                       Long.MIN_VALUE (or any timestamp prior to TSF) to
     *                       process all messages.
     * @param movePastTSFEnd If true, the reader will move to next time slice, as
     *                       normal. If false, the reader will return false from
     *                       {@link #readNext(deltix.qsrv.dtb.store.pub.TSMessageConsumer) }
     *                       at the end of the current time slice.
     * @param filter         The entity filter. Pass null to read all data.
     */
    public void open(TSRef timeSlice, long timestamp, boolean movePastTSFEnd, EntityFilter filter);

    /**
     * Resets this accessor to focus on the first message at or after the specified
     * timestamp.
     * 
     ...
     * @param filter    The entity filter. Pass null to read all data.
     */
    public void open(long timestamp, boolean forward, EntityFilter filter);
     ...
     *
     * @param timestamp The timestamp (all times are in nanoseconds) to limit
     *                  reader.
     */
