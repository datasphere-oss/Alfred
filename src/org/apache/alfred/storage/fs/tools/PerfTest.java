...
import org.apache.alfred.util.collections.PrintVisitor;
...
public class PerfTest implements Disposable {
    ...
    public void listSymbols() {
        ...
        for (int ii = 0; ii < n; ii++) {
            ...
            System.out.println("    " + sr.idToSymbol(id) + " #" + id + ": " + tr.from + " .. " + tr.to + " Data: ["
                    + sr.getEntityData(id) + "]");
        }
    ...
    public void generateMessages(int numEntities, int msgSize, long numMsgs, long timestampBatchSize,
            String compression) {
        ...
        for (int ii = 0; ii < numEntities; ii++) {
            symbols[ii] = "DLTX_" + ii;
            ids[ii] = root.getSymbolRegistry().registerSymbol(symbols[ii], "data for " + symbols[ii]);
        }
        ...
        try (DataWriter writer = cache.createWriter()) {
            ...
            TSMessageProducer producer = new TSMessageProducer() {
                @Override
            ...
            for (long count = 0; count < numMsgs; count++) {
                ...
                long timestamp = t + timestampBatchSize * (count / timestampBatchSize);

                writer.insertMessage(ids[entity], timestamp, 0, producer);
            }
        }
    ...
    public void readMessages(int numReaders, int readTime, EntityFilter[] filters) throws InterruptedException {
        root.open(true);
        ...
        for (int ii = 0; ii < numReaders; ii++)
            readers[ii] = new Reader(limit, filters == null ? null : filters[ii % filters.length]);
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int ii = 0; ii < args.length;) {
            ...
            switch (arg) {
                ...
                case "mt":
                    ...
                    break;
                case "-tb":
                    tsbs = Long.parseLong(args[ii++]);
                    break;
                case "-e":
                    numEntities = Integer.parseInt(args[ii++]);
                    break;
                case "-z":
                    compression = args[ii++];
                    break;
                case "-s":
                    msgSize = Integer.parseInt(args[ii++]);
                    break;
                case "-n":
                    numMessages = Long.parseLong(args[ii++]);
                    break;
                case "-d":
                    path = args[ii++];
                    break;
                case "-nr":
                    numReaders = Integer.parseInt(args[ii++]);
                    break;
                case "-rt":
                    readTime = 1000 * Integer.parseInt(args[ii++]);
                    break;
                case "-ef":
                    filters = parseFilters(args[ii++]);
                    break;
        ...
        if (write)
            FSUtils.removeRecursive(fs.createPath(path), false, new PrintVisitor(System.out, "rm ", "\n"));
