...
import org.apache.alfred.storage.fs.azure2.Azure2FS;
import org.apache.alfred.storage.fs.cache.CachingFileSystem;
import org.apache.alfred.storage.fs.chunkcache.ChunkCachingFileSystem;
...
public final class FSFactory {
    ...
    // if (ramCacheSize > 0) {
    // isCacheEnabled = config.getBoolean("cacheEnabled",
    // Boolean.valueOf(System.getProperty("TimeBase.cacheEnabled", "true")));
    //
    // if (isCacheEnabled)
    ...
    public static void init(int maxFileSize, long localFsCacheSizeInBytes, long dfsCacheSizeInBytes,
            double preallocateRatio) {
     ...
     * If you want to reset behavior to a default then you should use
     * {@code FSFactory.forceSetLocalFS(null)}.
     */
    @VisibleForTesting
    ...
    /**
     * This method allows to set local file system to a provided value. This method
     * will wrap provided FS as it usually done with {@link LocalFS}.
     *
     * WARNING: This should be used only for tests.
     *
     * WARNING: This method modifies global state so it will affect all interaction.
    ...
    private static AbstractFileSystem createDistributedFS(String scheme, String address) throws IOException {
        ...
        throw new IllegalArgumentException(
                "No supported FileSystem for scheme '" + scheme + "' (Address: '" + address + "').");
    }
