...
public interface AbstractPath {
    ...
    /**
     * Same as {@link #openInput(long)} but guaranties to load only first
     * {@code length} bytes of data (if available).
     *
    ...
    /**
     * @param size when greater than zero specifies output file size (strict), when
     *             zero - size is unknown (usually small)
     */
    @CheckReturnValue
    ...
    /**
     * This method can be use to change metadata caching policy. If caching is
     * turned on then some query operations will return result without querying the
     * remote system. However this may produce incorrect behavior for operations
     * that expect concurrent access.
     */
