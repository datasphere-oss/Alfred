...
/**
 * HeapManager implemented using "buddy" memory allocation algorithm.
 * Thread-safe
 */
public class BinaryBuddyHeapManager implements HeapManager {
    ...
    public synchronized boolean allocate(int size, ByteArray block) {
        ...
        if (blockOffset != BinaryBuddyAllocator2.NOT_AVAILABLE) {
            ...
            usedSize += getBlockSize(size); // TODO: Here and in other places: we calculate size twice: here and inside
                                            // Allocator :-(
    ...
    /**
     * @return false if given block *definitely* was not allocated by this
     *         allocator. If result is true, block might have been allocated or not.
     */
