...
import org.apache.alfred.util.lang.Assertions;
...
final class BinaryBuddyAllocator {
    ...
    /**
     * @param maxBlockSize maximum allocation unit (equals to entire heap size this
    ...
    /**
     * @return offset of allocated block of requested size (note allocated block may
     *         be larger than requested, but caller is not allowed to use extra
     *         space). Method returns {@link #NOT_AVAILABLE} if this allocator has
     *         no free space to accommodate this request.
     */
    ...
    /**
     * Find free block at given tier. If none found try to allocate half of block of
     * larger size
     * 
     * @param lessThanBlock if this parameter specified method must search only
     *                      among blocks of smaller address (result < lessThanBlock)
    ...
    private int allocateSmallerAddress(final int tier, final int lessThanBlock) {
        ...
        int block = blockTracker.removeAny(tier, lessThanBlock); // Check if we already have the right size block at
                                                                 // hand
        if (block == NOT_AVAILABLE && lessThanBlock == NOT_AVAILABLE) {
            // we need to split a bigger block
            block = allocateSmallerAddress(tier + 1, NOT_AVAILABLE); // TODO: Consider using lessThanBlock parameter
                                                                     // here... (v2)
    ...
    /**
     * Relocates given block to the "left" - to free block with the smallest
     * address.
     * 
     * @return new offset for the block. We expect caller to move data from block
     *         identified by input parameter to block identified by result. Method
     *         returns {@link #NOT_AVAILABLE} if this allocator has no free space to
    ...
    int defragment(int block, int size) {
        ...
        if (result != NOT_AVAILABLE) {
            if (DEBUG)
                LOG.log(LogLevel.DEBUG).append("Moved block of size ").append(size).append(" from #").append(block)
                        .append(" to #").append(result).commit();
    ...
    /**
     * Free Block Tracker organize blocks by tier: highest tier hold largest block,
     * tier 0 tracks smallest blocks
     */
    interface Tracker {
         ...
         * 
         * @param tier          remove first available block from free list of given
         *                      tier
         * @param lessThanBlock (if specified) than result should be less than
         * @return Returns block or NOT_AVAILABLE if there are no free blocks at given
         *         tier
         */
    ...
    /**
     * Tracker uses BitSet to allocate "address ordered" best fit buddies (lowest
    ...
    static final class BitSetTracker implements Tracker {
        ...
        /**
         * Bitmask of every tier, starting from tier 0 which has block size equal to 1
         * .. up until top tier that has only one block of size ( 1 << tier)
         */
        ...
        public void add(int tier, int block) {
            ...
            if (ASSERTIONS_ENABLED)
                assert !bitmask.get(blockOffset) : "unused block " + block + " at tier " + tier + " state:"
                        + toString();
        ...
        public int removeAny(int tier, int lessThanBlock) {
            ...
            final int lessThanOffset = (lessThanBlock != NOT_AVAILABLE) ? getBlockOffset(tier, lessThanBlock)
                    : Integer.MAX_VALUE;
        ...
        /**
         * @return Dump of tracker state in the following format:
         *         tier-number:[free-block1,free-block2,..]
         */
        @Override
        public String toString() {
            StringBuilder sb = new StringBuilder();
