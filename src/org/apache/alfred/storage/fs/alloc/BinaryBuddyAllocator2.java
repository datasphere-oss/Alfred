...
final class BinaryBuddyAllocator2 {
    ...
    /**
     *
     * @param minBlockSize minimum allocation unit (you can request a smaller size
     *                     but this implementation will return you a chunk of at
     *                     least minBlockSize size)
    ...
    int allocate(int size) {
        ...
        // LOG.level(Level.INFO).append("(C) Alloc (").append(size).append(") =
        // ").append(result).commit();
        return result;
    }

    void deallocate(int block, int size) {
        // LOG.level(Level.INFO).append("(C) Dealloc (").append(block).append(",
        // ").append(block).append(')').commit();
