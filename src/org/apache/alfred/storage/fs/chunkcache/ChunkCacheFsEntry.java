...
import org.apache.alfred.util.BitUtil;
...
final class ChunkCacheFsEntry {
    ...
    // Contract: if a thread want to download a chunk for this file then the thread
    // should acquire this lock and only then lock on ChunkCacheFsEntry.
    ...
    /**
     * @param expectedMaxFileSizeBytes This value defines upper estimate for
     *                                 "common" files. This is not a hard limit but
     *                                 accessing files bugger than this size will
     *                                 involve additional memory allocation and
     *                                 extra copying.
     */
    ...
    public int getSizeEstimate() {
        // Even if there no chunks yet usually that means that we going to add at least
        // one.
    ...
    /**
     * @param chunkPool free chunks (if any) will be released to this pool during
    ...
    // Note: there is a possibility of deadlock in case of concurrent renames A->B
    // and B->A. TODO: Decide how to fix.
    ...
    public synchronized void putChunk(int chunkIndex, Chunk chunk, int chunkDataLength, ChunkPool chunkPool) {
        ...
        chunk.addUse();
        assert chunk.getUsageCount() >= 2; // One use by the stream and one use by this cache entry. In case of rename 3
                                           // usages possible.
    ...
    /**
     * Returns number of missing (not loaded) sequential chunks starting from
     * {@code startChunkIndex} and up to {@code chunksToFulfillRequest}.
     */
