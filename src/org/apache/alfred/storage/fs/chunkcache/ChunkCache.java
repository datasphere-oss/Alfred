...
import org.apache.alfred.storage.fs.chunkcache.chunkpool.ChunkPool;
import org.apache.alfred.storage.fs.pub.AbstractPath;
...
class ChunkCache {
    ...
    // Value 0.3 means that if more than 30% of cache capacity invalidated for
    // whatever reason this memory will me released to JVM (and will be collected by
    // GC).
    ...
    ChunkCache(long cacheSizeInBytes, int expectedMaxFileSizeInBytes, double preallocateRatio) {
        // We set pool size to 1/4 of all cache size or maxFileSizeInBytes (whatever is
        // bigger)
        int poolEntryCount = Ints
                .saturatedCast(Math.max(Math.round(cacheSizeInBytes * POOL_RATIO), expectedMaxFileSizeInBytes)
                        / ChunkCachingFileSystem.CHUNK_SIZE);
        if (poolEntryCount < 16) {
            throw new IllegalArgumentException("Chunk pool is too small: " + poolEntryCount);
        }
        int chunkCountToPreallocate = Math.min(poolEntryCount, Ints
                .saturatedCast(Math.round(cacheSizeInBytes * preallocateRatio) / ChunkCachingFileSystem.CHUNK_SIZE));
        ...
        LOG.info("Assigning %sMb for Chunked FileSystem cache. Chunk pool: %s / %s (%sMb / %sMb)")
                .with(cacheSizeInBytes >> 20).with(chunkCountToPreallocate).with(poolEntryCount)
                .with(preallocatedSizeInBytes >> 20).with(poolSizeInBytes >> 20);

        this.chunkPool = new ChunkPool(poolEntryCount);
        this.chunkPool.preallocateChunks(chunkCountToPreallocate);

        this.cache = Caffeine.newBuilder().maximumWeight(cacheSizeInBytes)
    ...
    private boolean updateEntrySize(String key, ChunkCacheFsEntry updatedEntry) {
        ChunkCacheFsEntry currentEntry = cache.getIfPresent(key); // Note: this will refresh the entry. So it should not
                                                                  // expire during this method after that call
        if (currentEntry == updatedEntry) {
            // This is same entry. We should just call update on cache to update the value
            // "weight".
            ...
            // We have to a) merge data from old entry to new entry or b) just discard old
            // entry
    ...
    // Note: Rename process is not atomic. Multiple threads that attempt to rename
    // same file may observe inconsistent results.
    void rename(AbstractPath sourcePath, AbstractPath destinationPath) {
        ...
        if (sourceEntry != null && sourceEntry.getLoadedChunkCount() > 0) {
            ...
            // Note: we can't just reuse entry object because old entry object will be
            // invalidated so it's data will be cleaned (in the removalListener).
