...
package org.apache.alfred.storage.fs.chunkcache;

import org.apache.alfred.storage.fs.chunkcache.chunkpool.Chunk;
import org.apache.alfred.util.io.BasicIOUtil;
...
public class ChunkCacheInputStream extends InputStream {
    ...
    private long wrappedStreamLimit = -1; // if >= 0 then this is the limit for data in the requested stream. Limit
                                          // count from the file start (not stream start).
    ...
    private void changeChunk(int bytesRequested) throws IOException {
        ...
        // TODO: We can decrease synchronization scope but that will mean than two
        // threads may download same data
        ...
        // TODO: Provide separate lock object for downloads. So we can at least read
        // data in cache while other thread downloads chunks.
        boolean needToLoad;
        synchronized (entry) {
        ...
        if (needToLoad) {
            synchronized (entry.getLoadLock()) {
                ...
                if (needToLoad) {
                    ...
                    // Note: there is a chance that entry was evicted from the cache. We still
                    // update old entry.
                    synchronized (entry) {
    ...
    private void useDataFromCachedChunk(ChunkCacheFsEntry entry, int chunkIndex, long newChunkOffset,
            Chunk chunkObject) {
        releaseChunk();
        this.chunkObject = chunkObject;
        this.currentChunk = chunkObject.getArray();
        this.chunkOffset = newChunkOffset;
        this.chunkLimit = entry.getChunkLimitByIndex(chunkIndex); // Ensure that this call in same sync block with
                                                                  // "getChunk"
    }
    ...
    private int loadChunk(long newChunkOffset, int requestedChunksToLoad) throws IOException {
        ...
        if (wrappedStream != null) {
            ...
            if (reopenOnSeekThreshold != 0
                    && skipLength >= Math.min(reopenOnSeekThreshold, ChunkCacheFsEntry.CHUNK_SIZE)) {
                ...
                // We should NOT meet end of file here in normal operation because we skip to
                // the place where data should be present
                try {
        ...
        if (wrappedStream == null) {
            ...
            int numberOfChunksToRequest = determineOptimalNumberOfChunksToLoad(path, hadToJump, requestedChunksToLoad,
                    firstChunk);
    ...
    /**
     * @param hadToJump             if we ever needed to skip a chunk for this
     *                              stream?
     * @param requestedChunksToLoad number of chunks needed to fulfill current
     *                              request
     * @param firstChunk            is this the first chunk requested for this
     *                              stream
     * @return number of chunks to load or -1 if no limit
     */
    private static int determineOptimalNumberOfChunksToLoad(ChunkCachingAbstractPath path, boolean hadToJump,
            int requestedChunksToLoad, boolean firstChunk) {
    ...
    /**
     * @return number of bytes read. can be less than length only if end of stream
     *         reached
     */
    private static int readFully(InputStream is, byte[] bytes, int offset, int length) throws IOException {
        int bytesRead = 0;
        while (length > 0) {
