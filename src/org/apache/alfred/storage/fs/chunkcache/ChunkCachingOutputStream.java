...
import org.apache.alfred.qsrv.dtb.fs.chunkcache.chunkpool.Chunk;
import org.apache.alfred.qsrv.dtb.fs.chunkcache.chunkpool.ChunkPool;
...
class ChunkCachingOutputStream extends OutputStream {
    ...
    ChunkCachingOutputStream(OutputStream delegate, ChunkCachingAbstractPath path, ChunkCache cache,
        ...
        int expectedChunkCount = expectedSizeInBytes > 0
                ? Ints.checkedCast(expectedSizeInBytes / ChunkCachingFileSystem.CHUNK_SIZE)
                : 10;
    ...
    private void checkSize() {
        if (!sizeOverflow && offset > expectedSizeInBytes) {
            sizeOverflow = true;
            ChunkCache.LOG
                    .warn("File size is bigger than expected. Expected size: %s. %s bytes already written. Path: %s")
                    .with(expectedSizeInBytes).with(offset).with(path.getPathString());
        }
    }
