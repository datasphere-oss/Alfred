...
class PathImpl extends File implements AbstractPath {
        ...
        for (String s : components) {
            switch (s) {
                case ".":
                    break;
                case "..":
                    ret = ret.getParentPath();
                    break;
                default:
                    ret = new PathImpl(ret, s, fs);
                    break;
            }
        }
    ...
    static void fsync(Path fileToSync, boolean isDir) throws IOException {
        ...
        // must open r/w for the fsync to have an effect.
        // See
            ...
            try {
                ...
                if (isDir) {
                    assert !(Constants.LINUX
                            || Constants.MAC_OS_X) : "On Linux and MacOSX fsyncing a directory should not throw IOException, "
