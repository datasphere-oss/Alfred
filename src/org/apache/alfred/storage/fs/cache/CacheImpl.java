...
package org.apache.alfred.storage.fs.cache;

import com.google.common.util.concurrent.Striped;
import org.apache.alfred.gflog.api.*;
import org.apache.alfred.storage.fs.alloc.BinaryBuddyHeapManager;
import org.apache.alfred.storage.fs.alloc.HeapManager;
import org.apache.alfred.util.collections.ByteArray;
import org.apache.alfred.util.collections.CharSequenceToObjectMapQuick;
import org.apache.alfred.util.collections.QuickList;
import org.apache.alfred.util.time.GlobalTimer;
import org.apache.alfred.util.time.TimerRunner;
 ...
 *
 * a) Free - listed in vacant entries list, userCounter=0, pathString = null (no
 * content) b) Reusable - listed in vacant entries, userCount=0, pathString !=
 * null, registered in entriesByPath (has valid content) c) Used - not listed in
 * vacant entries, userCount > 0, pathString !=null, registered in entriesByPath
 * (has valid content)
 */
class CacheImpl implements Cache {
    ...
    /**
     * Cache of entries. Some of them could be currently checked out and some not
     * (in latter case they will be also in vacant list)
     */
    @GuardedBy("lock")
    ...
    /**
     * Pool of cache entries that may or may not have content. Ordered by most
     * recently used entries going last.
     *
     * Entries that have re-usable content have their
     * {@link CacheEntry#getPathString()} set to not null. Entries that do not have
     * pathString do not have any data buffer (it is returned to the heap).
     *
     * NOTE: all entries in this list have *zero* reference counter.
     */
    @GuardedBy("lock")
    private final QuickList<CacheEntryImpl> vacant = new QuickList<>(); // assert entry.pathString is null and
                                                                        // entry.userCount == 0
    ...
    public static CacheImpl create(int size, int maxEntrySize) {
        ...
        if (maxBlockSize > Integer.MAX_VALUE)
            throw new IllegalArgumentException(
                    new StringBuilder().append("Max block size is too large: ").append(maxBlockSize).toString());
    ...
    public CacheImpl(int size, int minBlockSize, int maxBlockSize) {
        ...
        LOG.log(LogLevel.INFO).append("Allocating ").append(maxBlockSize >> 20).append("Mb for FileSystem cache")
                .commit();
    ...
    public CacheEntry checkOut(CacheEntryLoader cacheEntryLoader) {
        ...
        /// LOG.level(Level.INFO).append("CheckOut(").append(pathString).append(")=").append(result).append("
        /// THREAD:"+Thread.currentThread().getId()).commit();
        return result;
    }
    ...
    public void checkIn(CacheEntry entry) {
        ...
        /// LOG.level(Level.INFO).append("CheckIn(").append(e.getPathString()).append("=").append(e).append("
        /// THREAD:"+Thread.currentThread().getId()).commit();
    }
    ...
    public void invalidate(String pathString) {
        synchronized (lock) {
            ...
            if (entry != null) {
                ...
                if (entry.userCounter == 0) { // NOTE: Otherwise is checked out by one or several readers - invalidate
                                              // only affects new readers
        ...
        if (fileLength <= heap.getHeapSize()) {
            ...
            if (allocate((int) fileLength, result.getBuffer())) {
                try {
                    ...
                    LOG.log(LogLevel.ERROR).append("Can't load file \"").append(pathString)
                            .append("\" - skipping cache").commit();
                    result.pathString = null;
                    heap.deallocate(result.getBuffer());
                }
            } else {
            ...
            LOG.log(LogLevel.INFO).append("File is too large to cache: \"").append(pathString)
                    .append("\" - skipping cache").commit();
            assert result.pathString == null;
        }
    }
    ...
    private static final class CacheEntryImpl extends QuickList.Entry<CacheEntryImpl> implements CacheEntry, Loggable {
        ...
        private int userCounter; // Counts number of current checkouts. Zero implies that this entry in vacant
                                 // list
        private final ByteArray data = new ByteArray();

        /**
         * Filename of currently loaded buffer or <code>null</code> if this entry is
         * vacant
         */
    ...
    private void scheduleExpirationJob() {
        ...
        GlobalTimer.INSTANCE.schedule(new EntryExpirationJob(maxCacheEntryLifetime), expirationJobInterval,
                expirationJobInterval); // TODO: Increase initial interval
    }
    ...
    private class EntryExpirationJob extends TimerRunner {
        ...
        protected void runInternal() throws Exception {
            ...
            if (releasedEntriesCount > 0)
                LOG.log(LogLevel.DEBUG).append("Cache cleaner released ").append(releasedEntriesCount)
                        .append(" entries").commit();
