...
import org.apache.alfred.storage.fs.common.DelegatingAbstractPath;
import org.apache.alfred.storage.fs.common.DelegatingOutputStream;
import org.apache.alfred.storage.fs.pub.AbstractFileSystem;
        ...
        if (exists()) {
            if (length() <= cfs.getMaxBufferSize()) {
                ...
                if (cacheEntry != null) {
                    ...
                    return new ByteArrayInputStream(data.getArray(), data.getOffset() + (int) offset, (int) length()) { // TODO:
                                                                                                                        // ObjectPool
                                                                                                                        // of
                ...
                LOG.log(LogLevel.WARN).append("Skipping cache for ").append(getPathString()).append(" filesize:")
                        .append(length()).commit();
            }
        }
    ...
    public void load(CacheEntry entry) throws IOException {
        ...
        if (Assertions.ENABLED)
            if (length() > entry.getBuffer().getLength())
                throw new IOException("Entry size " + entry.getBuffer().getLength() + " is not enough to load "
                        + length() + " bytes: " + getPathString());
        ...
        try (InputStream is = delegate.openInput(0)) {
            ...
            if (bufferRemaining > 0)
                LOG.log(LogLevel.WARN).append("Cache entry for [").append(getPathString())
                        .append("] experienced incomplete file read, remaining size:").append(bufferRemaining).commit();
        }
    }
