...
/**
 * Interface that helps re-allocate buffer and load file content into the buffer
 * of known size
 */
...
interface Cache {
    ...
    /**
     * @return A cache entry for given path string or <code>null</code> (if file
     *         cannot be read or too large to cache). This method increments
     *         CacheEntry reference counter, caller must call
     *         {@link #checkIn(CacheEntry)} to release returned entry (when not
     *         null).
     */
    CacheEntry checkOut(CacheEntryLoader cacheEntryLoader);

    /**
     * Method to release cache entry previously obtained from
     * {@link #checkOut(CacheEntryLoader)} call. This method decrements CacheEntry
     * reference counter.
     */
    ...
    // /** Preload given file path in cache (if cache has vacant spot). Preloading
    // happens in background. Preloaded pages initially have zero reference counter.
    // */
    ...
    /**
     * Allocates entry of given size for writing. It will not be visible to others
     * until caller calls {@link #update(String, CacheEntry)}.
     */
