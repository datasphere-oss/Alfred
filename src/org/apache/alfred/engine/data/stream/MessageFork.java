...
public abstract class MessageFork<T> implements MessageChannel<T> {
    ...
    public MessageFork(MessageChannel<T> acceptedMessageConsumer, MessageChannel<T> rejectedMessageConsumer) {
    ...
    public final void send(T message) {
        MessageChannel<T> consumer = accept(message) ? mAcceptedMessageConsumer : mRejectedMessageConsumer;
