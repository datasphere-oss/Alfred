...
/**
 * Hand-optimized priority queue of MessageSource objects. Analogous to
 * java.util.PriorityQueue &lt;MessageSource &lt;T&gt;&gt;
 */
public final class MessageSourcePriorityQueue<T extends TimeStampedMessage> {
    ...
    /**
     * The heap represented by array. Root has the index of 1. Left child of node A
     * is at A &lt;&lt; 1. Right child of node A is at A &lt;&lt; 1 + 1. Parent of
     * node A is at A &gt;&gt; 1.
     */
    ...
    public void assertValid() {
        for (int pos = 1; pos <= mSize; pos++) {
            ...
            if (leftChildPos <= mSize) {
                ...
                assert (child.getMessage().getNanoTime() >= obj.getMessage().getNanoTime()) : "(L) child at "
                        + leftChildPos + ": " + child + " < parent: " + obj + "; parent pos: " + pos;
            }

            if (rightChildPos <= mSize) {
                MessageSource<T> child = getInternal(rightChildPos);

                assert (child.getMessage().getNanoTime() >= obj.getMessage().getNanoTime()) : "(R) child at "
                        + rightChildPos + ": " + child + " < parent: " + obj + "; parent pos: " + 1;
            }
        }
    }
    ...
    /**
     * Spec: set heap at <code>pos</code> to <code>obj</code>, and percolate the
     * element up if necessary.
     */
    ...
    /**
     * Spec: set heap at <code>pos</code> to <code>id</code>, and percolate the
     * element down if necessary.
     */
    ...
    private void removeInternal(int pos) {
        /**
         * We are removing element at pos, so we are going to have a hole there. Put the
         * last element in the hole and adjust the heap.
         */
