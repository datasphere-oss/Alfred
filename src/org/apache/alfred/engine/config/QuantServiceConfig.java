...
import org.apache.alfred.qsrv.QSHome;
import org.apache.alfred.qsrv.SSLProperties;
import org.apache.alfred.qsrv.util.text.Mangle;
import org.apache.alfred.util.cmdline.DefaultApplication;
import org.apache.alfred.util.io.Home;
import org.apache.alfred.util.io.TokenReplacingReader;
import org.apache.alfred.util.lang.Depends;
import org.apache.alfred.util.lang.DependsOnClass;
import org.apache.alfred.util.lang.SortedProperties;
import org.apache.alfred.util.lang.StringUtils;
import org.apache.alfred.util.os.CommonSysProps;
...
/**
 * After setting up QSHome, this provides a small generic framework for
 * determining the host, port and other properties of a service.
 */
@Depends({ "config/logger.properties", "config/gflog.xml" })
...
public class QuantServiceConfig {
    ...
    public enum Type {
        TimeBase, UHF, Aggregator, QuantServer, ExecutionServer, StrategyServer
    }
    ...
    public static QuantServiceConfig forService(Type type) throws IOException {
        CommonSysProps.mergePropsOnceIfFileExists();

        return (new QuantServiceConfig(type));
    }

    public static QuantServiceConfig forApp(DefaultApplication app, Type type) throws IOException {
        CommonSysProps.mergePropsOnceIfFileExists();
    ...
    /**
     * Creates {@link QuantServiceConfig} with configuration populated from provided
     * {@link Properties} object.
     * <p>
     *
     * This method is supposed to be used from tests where you may want to construct
     * {@link QuantServiceConfig} programmatically (instead of reading it from
     * file).
     * <p>
     *
    ...
    public void setStringFromCmdLine(DefaultApplication app, String arg, String key, Object defaultValue)
            throws IOException {
        setStringFromCmdLine(app, arg, myType, key, defaultValue);
    }

    public void setStringFromCmdLine(DefaultApplication app, String arg, Type type, String key, Object defaultValue)
            throws IOException {
    ...
    public String getTBLoginPassword() {
        final String myServiceUser = getString(myType, TB_LOGIN_USER, null);
        return (myServiceUser != null) ? getString(myType, TB_LOGIN_PASS, null)
                : getString(Type.QuantServer, TB_LOGIN_PASS, null);
    }
    ...
    public static void loadProperties(File file, Properties props) throws IOException {
        try (TokenReplacingReader reader = new TokenReplacingReader(new FileReader(file),
                new PropertiesTokenReplacer())) {
            props.load(reader);
