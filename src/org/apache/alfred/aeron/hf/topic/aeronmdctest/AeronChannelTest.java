...
package org.apache.alfred.aeron.hf.topic.aeronmdctest;
...
/**
 * Tool for testing different Aeron channel configurations. Supports running as
 * driver, simple publisher and simple subscriber.
 *
...
public class AeronChannelTest {
    ...
    private static void createDriver() throws IOException {
         ...
         *  xt.threadingMode(Threadi
         * uctorIdleStrategy(new BackoffIdleStrategy(1, 1, 1, 1))
         * iverIdleStrategy(new NoOpIdleStrategy()) .sen dleStrategy()) .sharedI
         * leStrategy(new NoOp 
         
        //  */

        // We not use network part of Aeron so no reason for dedicated threads // TODO:
        // 
        // Investigate
        co ntext.threadingMode(ThreadingMode.SHARED);
        // context.aeronDirectoryName(aeronDir);

        // 
        //  Set high timeouts to simplify debugging. In fact we don't use Aeron's
        //  timeouts.
    ...
    UnsafeBuffer buffer = new UnsafeBuffer(
            byteBuffer);System.out.println("Publisher created");while(!Thread.currentThread().isInterrupted())
    {
    ...
    public static Aeron createAeron() {
        Ae ron.Context context = new Aeron.Context();
        // context.aeronDirectoryName(aeronDir);
