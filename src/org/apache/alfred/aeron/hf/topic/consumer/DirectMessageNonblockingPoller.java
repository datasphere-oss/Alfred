...
import org.apache.alfred.aeron.hf.topic.consumer.annotation.AeronClientThread;
import org.apache.alfred.aeron.hf.topic.consumer.annotation.AnyThread;
import org.apache.alfred.aeron.hf.topic.consumer.annotation.ReaderThreadOnly;
import org.apache.alfred.util.concurrent.CursorIsClosedException;
...
class DirectMessageNonblockingPoller implements MessagePoller {
    ...
    DirectMessageNonblockingPoller(Aeron aeron, boolean raw, String channel, int dataStreamId,
        ...
        DoubleUnavailableImageHandler unavailableImageHandler = new DoubleUnavailableImageHandler(fillChecker,
                DirectMessageNonblockingPoller.this::onUnavailableImage);

        // The main caveat here that we first create a subscription (so we start to
        // collect metadata)
        // and THEN get the mapping from the server using mappingProvider.
        // Otherwise it would be possible to get situation when we get stale data in the
        // server response
        ...
        LOG.debug("Got mapping snapshot for dataStreamId=%s of size=%s").with(dataStreamId)
                .with(mappingSnapshot.length);

        this.decodingFragmentHandler = new MessageFragmentHandler(raw, codecFactory, typeLoader, types, mappingSnapshot,
                mappingProvider);
    ...
    @ReaderThreadOnly
    @Override
    public int processMessages(int messageCountLimit, MessageProcessor messageProcessor)
            throws CursorIsClosedException {
        if (stopFlag) {
            handleStop();
        }
    ...
    /**
     * Closes allocated resources. Please not that this method must be called from
    ...
    private void onUnavailableImage(Image image) {
        ...
        if (!stopped && !stopFlag && decodingFragmentHandler != null
                && !decodingFragmentHandler.checkIfSessionGracefullyClosed(sessionId)) {
