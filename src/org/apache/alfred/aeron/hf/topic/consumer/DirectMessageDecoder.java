...
import org.apache.alfred.util.memory.MemoryDataInput;
...
import static org.apache.alfred.qsrv.hf.topic.DirectProtocol.*;
...
class DirectMessageDecoder {
    ...
    // If true then only metadata processed (all data messages are skipped). This
    // mode is used during the initialization process.
    // private boolean skipDataMode = false;

    /**
     * @param arrayBuffer     array buffer that works as source of data. It's
     *                        supposed to be shared with holding class
    ...
    /**
     * Processes single message from buffer. It that's a data message then returns
     * message value. If that's a metadata message then returns {@code null}.
     */
    @Nullable
    ...
    /*
     * void setSkipDataMode(boolean skipDataMode) { this.skipDataMode =
     * skipDataMode; }
     */

    /**
     * Checks if we got info from publisher that session is explicitly closed. After
     * the check data on that session gets removed. So if you call that method two
     * times with same sessionId then the second call will always return
     * {@code false}.
     *
    ...
    private void processIndexRemoved(int length) {
        mdi.setBytes(arrayBuffer.byteArray(), arrayBuffer.wrapAdjustment() + TEMP_INDEX_REMOVED_DATA_OFFSET,
                length - TEMP_INDEX_REMOVED_DATA_OFFSET);
        int recordCount = mdi.readInt();
        for (int i = 0; i < recordCount; i++) {
            int entityIndex = mdi.readInt();
    ...
    private void processEndOfStream(int length) {
        mdi.setBytes(arrayBuffer.byteArray(), arrayBuffer.wrapAdjustment() + END_OF_STREAM_DATA_OFFSET,
                length - END_OF_STREAM_DATA_OFFSET);
    ...
    private ConstantIdentityKey handleMissingTempIndex(int entityIndex) {
        ...
        IntegerToObjectHashMap<ConstantIdentityKey> tempMappingSnapshot = mappingProvider
                .getTempMappingSnapshot(entityIndex);
    ...
    private ConstantIdentityKey missingEntityIndex(int entityIndex) {
        // TODO: Handle the case when a new entity id is added just after the consumer
        // registration (but before consumer can start to listen data). In this case
        // consumer may miss mapping data.
        // This case is rare and may happen only during short period just after the
        // start but we still have to address it.
     ...
     *
     * @param mapping arrays of instrument keys, each key have same mapping index as
