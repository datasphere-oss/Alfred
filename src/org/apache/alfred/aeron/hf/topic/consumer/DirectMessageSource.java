...
import org.apache.alfred.util.concurrent.IntermittentlyAvailableCursor;
import org.apache.alfred.util.concurrent.NextResult;
...
class DirectMessageSource implements MessageSource<InstrumentMessage>, IntermittentlyAvailableCursor {
    ...
    // We align to long to make sure that we can read long value without alignment
    // issues
    ...
    // Contains multiple messages. Each message is 4 byte size header and binary
    // body/
    ...
    DirectMessageSource(Aeron aeron, boolean raw, String channel, int dataStreamId, CodecFactory codecFactory,
        ...
        this.decoder = new DirectMessageDecoder(arrayBuffer, raw, codecFactory, typeLoader, types, mappingSnapshot,
                mappingProvider);
    }
    ...
    public boolean next() throws CursorIsClosedException {
        ...
        while (true) {
            while (!hasBufferedMessages()) {
                if (!pageDataIn()) {
                    idleStrategy.idle();
                    // We must re-check closed status because it might have changed during the idle
                    // period.
                    assertNotClosed();
                } else {
                    idleStrategy.reset();
    ...
    // Can be executed from different thread. So it can be used to stop message
    // processing executed by one thread from another.
    @AnyThread
    ...
    private class FragmentHandler implements ControlledFragmentHandler {
        ...
        public Action onFragment(DirectBuffer buffer, int offset, int length, Header header) {
            ...
            if (remainingSpace() >= spaceNeeded) {
                ...
                // We are out of space
                if (hasBufferedMessages()) {
                    // We already have some buffered messages so we can abort loading current
